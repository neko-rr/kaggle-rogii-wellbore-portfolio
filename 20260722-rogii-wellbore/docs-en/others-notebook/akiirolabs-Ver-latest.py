# extracted from akiirolabs-tvt-prediction-model.ipynb

The Challenge. Predicting True Vertical Thickness (TVT) in horizontal wellbores represents a high-stakes stratigraphic inference problem. The gamma-ray (GR) signature, while useful, is notoriously ambiguous; repeating geological sequences frequently induce "tracking drift," where the regressor loses its stratigraphic reference point, resulting in non-physical trajectory anomalies. Because the competition metric is RMSE, even isolated instances of such drift can catastrophically degrade the total score.

Our Methodology. To ensure robustness, we deploy a diverse-ensemble engine that minimizes correlated bias. Rather than relying on a single estimator, we integrate:

Geometric-Spatial Priors: Incorporating offset-well data to ground the regression.

Tracker Diversity: A multi-method stack leveraging particle filters for signal likelihood, beam search for sequence consistency, and gradient-boosted ensembles (LGBM/CatBoost) for non-linear feature mapping.

Gated Recovery: A secondary pass that enforces stratigraphic logic, preventing branch-locking through a strict gate-and-recover heuristic.
%%writefile kaggle_train_and_submit.py
#!/usr/bin/env python3
"""ROGII TVT Kaggle entrypoint.

Locates the project pipeline script in the working directory or attached inputs
and executes it as the notebook run entrypoint.
"""
...
def find_pipeline_script() -> Path:
    candidates = [
        Path.cwd() / "rogii_train_and_submit.py",
        Path("/kaggle/working/rogii_train_and_submit.py"),
    ]

    input_root = Path("/kaggle/input")
    if input_root.exists():
        candidates.extend(input_root.glob("**/rogii_train_and_submit.py"))
    ...
    raise FileNotFoundError(
        "Could not find rogii_train_and_submit.py. Place it beside this entrypoint "
        "or attach it as a Kaggle input dataset."
    )


if __name__ == "__main__":
    runpy.run_path(str(find_pipeline_script()), run_name="__main__")

%%writefile rogii_train_and_submit.py
#!/usr/bin/env python3
"""ROGII wellbore TVT submission pipeline.

Runs the active ensemble, artifact-loading, blending, and submission-audit flow
for Kaggle execution.
"""

"""Runtime, logging and output layout."""
import builtins
import json
import os
import sys
...
LOCAL_KAGGLE_INPUT_ROOT = SCRIPT_DIR / "data" / "kaggle_input"
KAGGLE_INPUT_ROOT = Path(os.environ.get("ROGII_KAGGLE_INPUT_ROOT", "/kaggle/input"))
if not KAGGLE_INPUT_ROOT.exists() and LOCAL_KAGGLE_INPUT_ROOT.exists():
    KAGGLE_INPUT_ROOT = LOCAL_KAGGLE_INPUT_ROOT
KAGGLE_WORKING_ROOT = Path(os.environ.get("ROGII_KAGGLE_WORKING_ROOT", "/kaggle/working"))

def find_input_dir(*relative_candidates: str, marker: str | None = None, marker_parent_levels: int = 1) -> Path:
    """Resolve Kaggle input folders across local mirrors and Kaggle mount layouts."""
    for relative in relative_candidates:
        candidate = KAGGLE_INPUT_ROOT / relative
        if candidate.exists():
            return candidate
    if marker:
        matches = sorted(KAGGLE_INPUT_ROOT.glob(f"**/{marker}"))
        if matches:
            resolved = matches[0]
            for _ in range(marker_parent_levels):
                resolved = resolved.parent
            return resolved
    return KAGGLE_INPUT_ROOT / relative_candidates[0]

class NotebookLogger:
    """Small readable logger for notebook stages."""
    ...
    def info(self, *parts, **kwargs):
        _ORIGINAL_PRINT("  •", *parts, **kwargs)

    def success(self, *parts, **kwargs):
        _ORIGINAL_PRINT("  ✓", *parts, **kwargs)

    def warn(self, *parts, **kwargs):
        _ORIGINAL_PRINT("  !", *parts, **kwargs)

    def raw(self, *parts, **kwargs):
        _ORIGINAL_PRINT("  ·", *parts, **kwargs)

LOGGER = NotebookLogger()
RUN_DIAGNOSTICS = {
    "reported_baseline_score": "7.5",
    "diagnostics_version": 2,
    "model_load_events": [],
}

def print(*args, **kwargs):
    LOGGER.raw(*args, **kwargs)

def record_diagnostic(key, value):
    RUN_DIAGNOSTICS[key] = value

def append_diagnostic_event(event):
    RUN_DIAGNOSTICS.setdefault("model_load_events", []).append(event)

def count_paths(pattern):
    return len(glob.glob(str(pattern), recursive=True))

def write_run_diagnostics():
    path = OUTPUT.path("audit", "run_reproducibility_diagnostics.json")
    with open(path, "w", encoding="utf-8") as f:
        json.dump(RUN_DIAGNOSTICS, f, indent=2, sort_keys=True, default=str)
    return path

@dataclass(frozen=True)
class OutputManager:
    """Keeps submission in work root and auxiliary files in folders."""
    ...
    def create(cls):
        if KAGGLE_WORKING_ROOT.exists():
            ...
            if cwd.name == 'notebooks':
                root = cwd.parent / 'working'
            elif (cwd / 'competitions/public-comp/wellbore-geology-prediction').exists():
                root = cwd / 'competitions/public-comp/wellbore-geology-prediction/working'
            else:
                root = cwd / 'working'
        root.mkdir(parents=True, exist_ok=True)
        for name in ('models', 'reports', 'intermediate', 'audit'):
    ...
    def organize_known_outputs(self):
        moves = {
            'sp45_projection_submission.csv': self.path('intermediate', 'sp45_projection_submission.csv'),
            'fleongg_pretrained_submission.csv': self.path('intermediate', 'fleongg_pretrained_submission.csv'),
            'sp45_fleongg_blend_report.csv': self.path('reports', 'sp45_fleongg_blend_report.csv'),
            'guarded_overlap_override_report.csv': self.path('reports', 'guarded_overlap_override_report.csv'),
            'exact_overlap_override_report.csv': self.path('reports', 'exact_overlap_override_report.csv'),
            'gold_prefix_calibration_report.csv': self.path('reports', 'gold_prefix_calibration_report.csv'),
            'gold_prefix_cut_report.csv': self.path('reports', 'gold_prefix_cut_report.csv'),
            'version2_submission_audit.json': self.path('audit', 'version2_submission_audit.json'),
            'submission_version2_refactor_copy.csv': self.path('audit', 'submission_version2_refactor_copy.csv'),
            'submission_audit_copy.csv': self.path('audit', 'submission_audit_copy.csv'),
            'latest_valid_submission.csv': self.path('audit', 'latest_valid_submission.csv'),
        }
        import shutil
...
LOGGER.section('Notebook output layout')
LOGGER.success(f'work root: {OUTPUT.root}')
LOGGER.info('main submission:', OUTPUT.submission)


class KoolboxLoader:
    """Loads the offline koolbox package when Kaggle mounted it."""

    def __init__(self, logger):
        self.logger = logger

    def run(self):
        self.logger.section('Koolbox setup')
        kb_dir = str(KAGGLE_INPUT_ROOT / 'koolbox-offline')
        if not os.path.isdir(kb_dir):
            candidates = glob.glob(str(KAGGLE_INPUT_ROOT / '**' / 'koolbox*'), recursive=True)
            self.logger.info('candidates:', candidates[:5])
            if candidates:
                kb_dir = candidates[0]
        self.logger.info('using:', kb_dir)
        if os.path.isdir(kb_dir):
            whls = glob.glob(f'{kb_dir}/**/*.whl', recursive=True)
            if whls:
                for wheel in whls:
                    self.logger.info('install wheel:', wheel)
                    subprocess.run([sys.executable, '-m', 'pip', 'install', '--no-deps', wheel], check=False)
            else:
        ...
        import koolbox
        self.logger.success('koolbox:', koolbox.__file__)

KoolboxLoader(LOGGER).run()
...
class CFG:
    dataset_path = find_input_dir(
        "competitions/rogii-wellbore-geology-prediction",
        "rogii-wellbore-geology-prediction",
        marker="sample_submission.csv",
        marker_parent_levels=1,
    )
    artifacts_path = find_input_dir(
        "datasets/ravaghi/wellbore-geology-prediction-artifacts",
        "wellbore-geology-prediction-artifacts",
        marker="data/train.csv",
        marker_parent_levels=2,
    )
...
LOGGER.section('Reproducibility diagnostics')
record_diagnostic("kaggle_input_root", str(KAGGLE_INPUT_ROOT))
record_diagnostic("kaggle_working_root", str(KAGGLE_WORKING_ROOT))
record_diagnostic("dataset_path", str(CFG.dataset_path))
record_diagnostic("dataset_path_exists", CFG.dataset_path.exists())
record_diagnostic("artifacts_path", str(CFG.artifacts_path))
record_diagnostic("artifacts_path_exists", CFG.artifacts_path.exists())
record_diagnostic("sample_submission_exists", (CFG.dataset_path / "sample_submission.csv").exists())
record_diagnostic("artifact_train_csv_exists", (CFG.artifacts_path / "data" / "train.csv").exists())
record_diagnostic("artifact_lightgbm_pickle_count", count_paths(CFG.artifacts_path / "models" / "lightgbm-*" / "*.pkl"))
record_diagnostic("artifact_catboost_pickle_count", count_paths(CFG.artifacts_path / "models" / "catboost-*" / "*.pkl"))
record_diagnostic("expected_accelerator", "GPU T4 x2")
try:
    _gpu_check = subprocess.run(["nvidia-smi", "-L"], check=False, capture_output=True, text=True)
    record_diagnostic("nvidia_smi_available", _gpu_check.returncode == 0)
    record_diagnostic("nvidia_smi_output", (_gpu_check.stdout or _gpu_check.stderr).strip()[:1000])
except Exception as _gpu_error:
    record_diagnostic("nvidia_smi_available", False)
    record_diagnostic("nvidia_smi_output", str(_gpu_error))
LOGGER.info("dataset_path:", CFG.dataset_path)
LOGGER.info("artifacts_path:", CFG.artifacts_path)
LOGGER.info("artifact lightgbm pkl count:", RUN_DIAGNOSTICS["artifact_lightgbm_pickle_count"])
LOGGER.info("artifact catboost pkl count:", RUN_DIAGNOSTICS["artifact_catboost_pickle_count"])
LOGGER.info("GPU visible:", RUN_DIAGNOSTICS["nvidia_smi_available"])


"""Physics selector helpers."""
...
"""Feature builders and particle-filter kernels."""
SEED=42
...
def _beam_jit(sgr, tw_gr, si, BS, mc, es):
    """Beam search over local TVT index deltas, Numba JIT."""
...
class SP45EnsembleStage:
    """Builds the first stack, selector blend and root submission."""
    ...
    def run(self):
        global train_df, test_df, features, X, y, g, X_test
        global lgb_params, cb_params, ridge_params, pp_params
        global oof_preds, test_preds, overall_scores, fold_scores
        global ridge_trainer, ridge_oof_preds, ridge_test_preds
        global test_df2, sub_1, sub_2, sub
        record_diagnostic("sp45_started", True)
        record_diagnostic("sp45_artifact_train_csv_used", (CFG.artifacts_path / "data" / "train.csv").exists())
        if (CFG.artifacts_path / "data" / "train.csv").exists():
            train_df = pd.read_csv(CFG.artifacts_path / "data" / "train.csv", low_memory=False)
        else:
            LOGGER.warn("SP45 artifact train.csv missing; falling back to feature build from competition train files")
        ...
        for i, params in enumerate(lgb_params):
            ...
            if (CFG.artifacts_path / save_path).exists():
                ...
                trainer_paths = list((CFG.artifacts_path / save_path).glob('*.pkl'))
                append_diagnostic_event({
                    "stage": "sp45",
                    "model": f"lightgbm-{i+1}",
                    "mode": "load_artifact",
                    "path": str(CFG.artifacts_path / save_path),
                    "pickle_count": len(trainer_paths),
                })
                trainer = joblib.load(trainer_paths[0])

                print(f"Loaded lightgbm-{i+1} with overall RMSE: {trainer.overall_score:.4f}\n")
            else:
                LOGGER.warn(f"Missing artifact for lightgbm-{i+1}; fallback training will run")
                append_diagnostic_event({
                    "stage": "sp45",
                    "model": f"lightgbm-{i+1}",
                    "mode": "fallback_train",
        ...
        for i, params in enumerate(cb_params):
            ...
            if (CFG.artifacts_path / save_path).exists():
                ...
                LOGGER.warn(f"Missing artifact for catboost-{i+1}; fallback training will run")
                append_diagnostic_event({
                    "stage": "sp45",
                    "model": f"catboost-{i+1}",
                    "mode": "fallback_train",
        ...
        sub.to_csv(OUTPUT.submission, index=False)
        sub
        record_diagnostic("sp45_submission_rows", int(len(sub)))
        record_diagnostic("sp45_oof_models", sorted(overall_scores.keys()))
        record_diagnostic(
            "sp45_any_fallback_training",
            any(event.get("mode") == "fallback_train" for event in RUN_DIAGNOSTICS.get("model_load_events", [])),
        )
        self.logger.success(f'base submission rows={len(sub)} -> {self.output.submission}')
        return sub

LOGGER.section('SP45 ensemble')
sp45_submission = SP45EnsembleStage(LOGGER, OUTPUT).run()


class ProjectionPostprocessor:
    """Applies the robust low-order per-well projection layer."""
    ...
    def run(self):
        ...
        try:
            _base = _pd.read_csv(OUTPUT.submission)
            ...
            _final[['id','tvt']].to_csv(OUTPUT.submission, index=False)
            record_diagnostic("projection_applied_wells", int(_n_ok))
            record_diagnostic("projection_skipped", False)
            print('wrote projected submission.csv', _final.shape)
        except Exception as _e:
            record_diagnostic("projection_skipped", True)
            record_diagnostic("projection_error", str(_e))
            print('Projection skipped; retained blended submission:', _e)
        self.logger.success('projection layer finished')

LOGGER.section('Projection post-processing')
ProjectionPostprocessor(LOGGER, OUTPUT).run()


class ScoreReporter:
    """Displays compact CV diagnostics."""
...
LOGGER.section('CV diagnostics')
ScoreReporter().run()


class IntermediateSubmissionWriter:
    """Stores named intermediate submissions away from the root file."""

    def run(self):
        from pathlib import Path as _BlendPath
        import pandas as _blend_pd
        _sp45_path = OUTPUT.root / 'submission.csv'
        _sp45_df = _blend_pd.read_csv(_sp45_path)
        _sp45_df.to_csv(OUTPUT.root / 'intermediate' / 'sp45_projection_submission.csv', index=False)
        record_diagnostic("sp45_projection_rows", int(len(_sp45_df)))
        print('saved sp45_projection_submission.csv', _sp45_df.shape, flush=True)

LOGGER.section('Intermediate outputs')
IntermediateSubmissionWriter().run()


"""Fleongg stack helpers."""
...
def _find_data():
    for c in [KAGGLE_INPUT_ROOT / "competitions" / "rogii-wellbore-geology-prediction",
              KAGGLE_INPUT_ROOT / "rogii-wellbore-geology-prediction"]:
        if Path(c).exists() and (Path(c)/"train").exists():
            return Path(c)
    for p in glob.glob(str(KAGGLE_INPUT_ROOT / "**" / "train"), recursive=True):

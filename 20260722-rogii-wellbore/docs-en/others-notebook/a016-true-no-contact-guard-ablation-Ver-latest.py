# Extracted from a016-true-no-contact-guard-ablation (a016-true-no-contact-guard-ablation.ipynb)
...
# A016 v2 ablation: skip guarded contact override only; all other A013 settings unchanged.
DISABLE_GUARDED_CONTACT_OVERRIDE = True
os.environ["DISABLE_GUARDED_CONTACT_OVERRIDE"] = "1" if DISABLE_GUARDED_CONTACT_OVERRIDE else "0"
...
_DISABLE_GUARDED_CONTACT_OVERRIDE = _ov_os.environ.get('DISABLE_GUARDED_CONTACT_OVERRIDE', '0') == '1'
if _DISABLE_GUARDED_CONTACT_OVERRIDE:
    print('GUARDED override DISABLED (A016 ablation); kept the blend')
else:
    try:
...
if not _GOLD_ENABLE:
    ...
    for _profile_name in ('conservative', 'balanced', 'aggressive'):
        ...
        if _gold_os.environ.get('DISABLE_GUARDED_CONTACT_OVERRIDE', '0') == '1':
            print('gold guarded contact override DISABLED (A016 ablation)')
        else:
...
# A013 final submit-safe guard
# This cell guarantees that /kaggle/working/submission.csv exists at notebook end.
import hashlib as _a013_hashlib
import json as _a013_json
import shutil as _a013_shutil
from pathlib import Path as _A013Path

import numpy as _a013_np
import pandas as _a013_pd

_A013_WORK = _A013Path('/kaggle/working') if _A013Path('/kaggle/working').exists() else _A013Path('.')
_A013_WORK.mkdir(parents=True, exist_ok=True)
_A013_FINAL = _A013_WORK / 'submission.csv'

# Resolve competition data path defensively.
_A013_DATA = None
try:
    _A013_DATA = getattr(CFG, 'DATA', getattr(CFG, 'dataset_path', None))
except Exception:
    _A013_DATA = None

if _A013_DATA is None:
    for _cand in [
        _A013Path('/kaggle/input/competitions/rogii-wellbore-geology-prediction'),
        _A013Path('/kaggle/input/rogii-wellbore-geology-prediction'),
    ]:
        if (_cand / 'sample_submission.csv').exists():
            _A013_DATA = _cand
            break

_A013_DATA = _A013Path(_A013_DATA) if _A013_DATA is not None else None
_A013_SAMPLE = _A013_DATA / 'sample_submission.csv' if _A013_DATA is not None else None

def _a013_sha256(path):
    _h = _a013_hashlib.sha256()
    with open(path, 'rb') as _f:
        for _chunk in iter(lambda: _f.read(1 << 20), b''):
            _h.update(_chunk)
    return _h.hexdigest()

def _a013_read_valid(path, sample=None):
    try:
        df = _a013_pd.read_csv(path)
        if list(df.columns) != ['id', 'tvt']:
            return None, f'bad_columns_{list(df.columns)}'
        if not _a013_np.isfinite(df['tvt'].to_numpy(dtype=float)).all():
            return None, 'non_finite_tvt'
        df['id'] = df['id'].astype(str)
        df['tvt'] = df['tvt'].astype(float)
        if sample is not None:
            if len(df) != len(sample):
                return None, f'bad_len_{len(df)}_expected_{len(sample)}'
            if not df['id'].astype(str).equals(sample['id'].astype(str)):
                return None, 'id_order_mismatch'
        return df[['id', 'tvt']].copy(), 'ok'
    except Exception as e:
        return None, 'read_failed_' + str(e)[:120]

sample = None
if _A013_SAMPLE is not None and _A013_SAMPLE.exists():
    sample = _a013_pd.read_csv(_A013_SAMPLE)[['id']].copy()
    sample['id'] = sample['id'].astype(str)

candidate_paths = []
for name in [
    'submission.csv',
    'submission_sp45_fleongg_w0.60.csv',
    'submission_public_self_verified.csv',
...
# Add any other submission-like CSVs, newest first, but keep priority files above.
extra = sorted(
    [p for p in _A013_WORK.glob('submission*.csv') if p not in candidate_paths],
...
for p in candidate_paths:
    df, status = _a013_read_valid(p, sample=sample)
    candidate_report.append({'file': str(p), 'status': status})
    if df is not None:
        selected_path = p
        selected_df = df
        break
...
if selected_df is None:
    # Emergency fallback: always create a valid-format file to avoid Submission File Not Found.
    if sample is None:
        raise RuntimeError('A013 cannot build emergency fallback because sample_submission.csv was not found.')

    emergency_used = True
    selected_path = _A013_WORK / 'submission_A013_emergency_sample_fallback.csv'
    selected_df = sample.copy()

    # Try a minimal TVT fallback from visible prefixes per well; otherwise use 0.0.
    tvt_values = []
    for _id in selected_df['id'].astype(str):
        _well = _id[:8]
        _idx = int(_id[9:])
        _val = 0.0
        try:
            _hw = _a013_pd.read_csv(_A013_DATA / 'test' / f'{_well}__horizontal_well.csv')
            if 'TVT_input' in _hw.columns:
                _known = _hw['TVT_input'].dropna()
                if len(_known):
                    _val = float(_known.iloc[-1])
        except Exception:
            pass
        tvt_values.append(_val)
    selected_df['tvt'] = tvt_values
    selected_df[['id', 'tvt']].to_csv(selected_path, index=False)
    candidate_report.append({'file': str(selected_path), 'status': 'emergency_created'})

# Optional micro correction is deliberately disabled by default because submit safety is the priority.
# The selected public candidate is copied exactly.
selected_df[['id', 'tvt']].to_csv(_A013_FINAL, index=False)

# Final hard audit.
final_df, final_status = _a013_read_valid(_A013_FINAL, sample=sample)
if final_df is None:
    # Last-resort rewrite from selected_df with sample id order.
    if sample is not None and len(selected_df) == len(sample):
        tmp = sample.merge(selected_df, on='id', how='left')
        tmp['tvt'] = tmp['tvt'].astype(float).fillna(float(selected_df['tvt'].median()))
        tmp[['id', 'tvt']].to_csv(_A013_FINAL, index=False)
        final_df, final_status = _a013_read_valid(_A013_FINAL, sample=sample)

if final_df is None:
    raise RuntimeError('A013 final hard audit failed: ' + str(final_status))

audit = {
    'notebook': 'A013_rogii_public7091_submit_safe_micro_guard',
    'strategy': 'submit-safe public 7.091 guard: choose first valid public candidate and always write /kaggle/working/submission.csv',
    'selected_source': str(selected_path),
    'emergency_used': bool(emergency_used),
    'final_sha': _a013_sha256(_A013_FINAL),
    'rows': int(len(final_df)),
    'columns': list(final_df.columns),
    'tvt_min': float(final_df['tvt'].min()),
    'tvt_mean': float(final_df['tvt'].mean()),
    'tvt_max': float(final_df['tvt'].max()),
    'candidate_report': candidate_report,
    'final_status': final_status,
}
(_A013_WORK / 'A013_final_audit.json').write_text(_a013_json.dumps(audit, indent=2, sort_keys=True), encoding='utf-8')

# Print directory proof so Kaggle logs show the file exists.
print('A013 wrote final file:', str(_A013_FINAL))
print('A013 final file exists:', _A013_FINAL.exists())
print('A013 final file size:', _A013_FINAL.stat().st_size if _A013_FINAL.exists() else -1)

print('===== COPY THIS TO GPT START =====')
print('# A013 Run Summary')
print('- strategy: `submit-safe public 7.091 guard; no risky final correction; guaranteed /kaggle/working/submission.csv`')
print(f"- selected_source: `{audit['selected_source']}`")
print(f"- emergency_used: `{audit['emergency_used']}`")
print(f"- final_sha: `{audit['final_sha']}`")
print(f"- rows: `{audit['rows']}`")
print(f"- tvt_min: `{audit['tvt_min']:.6f}`")
print(f"- tvt_mean: `{audit['tvt_mean']:.6f}`")
print(f"- tvt_max: `{audit['tvt_max']:.6f}`")
print()
print('## A013 candidate report')
print(_a013_pd.DataFrame(candidate_report).to_string(index=False))
print()
print('## A013 manifest')
print(_a013_json.dumps(audit, indent=2, sort_keys=True))

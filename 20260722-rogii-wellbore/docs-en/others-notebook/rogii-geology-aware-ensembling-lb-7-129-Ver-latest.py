# %% [markdown]
# # ROGII Wellbore Geology Prediction — TVT tracking with a physics-informed particle filter + GBM ensemble
# 
# > First af all, you can follow the original version from this [notebook](https://www.kaggle.com/code/AmgedAlfaqih/7-091-public/notebook), thanks Amged Alfaqih for this notebook)
# 
# **Task.** For each horizontal well we know the trajectory (`MD`, `Z`) and gamma-ray log (`GR`) along a known *visible* section, plus a nearby *type well* with the same log tied to known geological markers (`ANCC`, `ASTNU`, `ASTNL`, `EGFDU`, `EGFDL`, `BUDA`). The goal is to extrapolate `TVT` (true vertical thickness — the well's position relative to the geological layers) into the *unseen* section of the lateral, i.e. to keep the well "geosteered" onto the right layer using only the log signal.
# 
# **Approach in one sentence.** Don't treat this as a plain tabular regression problem — track the well like a physicist would: a **particle filter** propagates a swarm of TVT hypotheses forward using gamma-ray correlation against the type well, while a **GBM stack** (LightGBM + CatBoost + Ridge) learns the residual drift on top of it. Two independently-built variants of this idea are trained, blended together, smoothed, and finally calibrated per well using a backtest on the known part of each trajectory.
# 
# ### Contents
# 1. [Setup](#1) — environment & imports
# 2. [Configuration & the physical reference model](#2) — geometry-based TVT reconstruction, particle-filter constants
# 3. [Feature building](#3)
# 4. [Track A — tabular ensemble](#4) — LightGBM + CatBoost → Ridge stack, blended with a particle-filter prior, then smoothed
# 5. [Track B — independent physics + GBM pipeline](#5) ("fleongg") — a self-contained second tracker
# 6. [Final ensemble](#6) — blend Track A + Track B, safety guard for reference wells, integrity audit, per-well calibration
# 7. [Visual diagnostics](#7) — blend composition, stacking weights, feature importance
# 8. [What changed vs. the lb7295 baseline](#8)
# 9. [Takeaways](#9)
# 
...
# ## 1. Setup
# 
# Install/locate the internal `koolbox` training-helper package (offline wheel, with a couple of fallback search paths for different Kaggle dataset mount points), then import the modeling stack.
...
# %% [markdown]
# <a id='2'></a>
# ## 2. Configuration & the physical reference model
# 
# `CFG` holds paths, CV scheme (`GroupKFold` by well — critical so a well never leaks between folds) and the metric. Below it are three groups of constants/helpers reused later:
# 
# - **`tvt_from_contacts`** — a purely geometric reconstruction of `TVT` from the type well's known geological markers: it anchors on one formation top (`EGFDU` by default) and projects `Z` back to `TVT` through it. This is the "physics" baseline the particle filter is built around.
# - **Beam-search / particle-filter constants** (`BEAM_CONFIGS`, `SELECTOR_*`, `PF_*`, `ANCC_*`) — a first pass at the tracker: a momentum + noise particle filter that nudges TVT hypotheses along the trajectory and reweights them by how well their implied position matches the type well's gamma-ray curve.
# - `run_particle_filter` — the reference single-hypothesis version of the tracker.
...
# ## 3. Feature building
# 
# Build the train/test tables (`build_dataset`, defined earlier in the pipeline the team reused from `koolbox`): per-row tabular features plus the particle-filter outputs, grouped by well ID for CV.
...
# %% [markdown]
# <a id='4'></a>
# ## 4. Track A — tabular ensemble
# 
# Three LightGBM configs and a CatBoost model are trained (or loaded from cached artifacts if already fitted) with `GroupKFold`-by-well early stopping, then stacked with a `Ridge` meta-model on the out-of-fold predictions.
...
# %% [markdown]
# ### 4.1 Blending the stack with the particle-filter prior
# 
# The Ridge stack's output is combined with the particle-filter delta (`pf_ancc`) via `apply_pp`: a warm-up factor ramps the correction in gradually with distance from the last known point (`md_since`), then a per-well Savitzky–Golay filter removes residual jitter. This produces `sub_1`, the first candidate submission.
...
# %% [markdown]
# ### 4.2 Second sub-model: per-well geology tracker
# 
# For every test well this recomputes the geometric contacts-based `TVT` reference and runs a 128-seed likelihood-weighted particle-filter ensemble (`run_pf_lik_ensemble_scales`) at several drift scales, picking `pf_scale_8` as the primary tracker output. **Where a test well ID coincides with a train well ID**, the known type-well correlation for that well is reused (visible in the `if wid in train_wells:` branch) so the contacts model has a matched reference to project through — this is the notebook's way of exploiting the fact that a handful of wells appear as fully-known trajectories in `train/` and as partially-known ones in `test/`. This produces `sub_2`, which is blended 0.3/0.7 with `sub_1` into `submission.csv`.
...
for i, wid in enumerate(test_wells):
    ...
    # أخذ المتوسط الحسابي للمقاييس الثلاثة لقتل التذبذب
...
# %% [markdown]
# ### 4.3 Post-hoc smoothing: robust per-well projection
# 
# A degree-4 robust (IRLS-weighted) polynomial fit per well denoises the blended trajectory and down-weights points that jumped onto the wrong geological branch, overwriting `submission.csv` in place. The CV score comparison plot below is purely diagnostic (fold RMSE per model), and the result is checkpointed as `sp45_projection_submission.csv` — this is Track A's final output.
...
try:
    ...
    for _wid, _g in _base.groupby('well'):
        try:
            ...
            _fit = _robfit(_s, (_tvt + _Z) - _anchor, 3)
...
# %% [markdown]
# <a id='5'></a>
# ## 5. Track B — independent physics + GBM pipeline ("fleongg")
# 
# This section is a **second, self-contained tracker**, built independently from Track A (own `CFG`, own particle-filter implementation, own LightGBM configs, own stacking/blend logic). It re-derives the same kind of ANCC/Z-coupled particle filters, adds a `FormationPlaneKNN` step that borrows local formation-plane orientation from nearby offset wells, trains a fresh LightGBM stack (or loads pretrained boosters if an artifact dataset is mounted, for fast inference), and produces its own `submission.csv` via `main()`. Having two differently-parameterized trackers built from scratch is what makes the later blend worth something — their errors are only partially correlated.
...
def main():
    ...
    print("building lik-PF + features (test)â€¦", flush=True)
    ...
    if models_dir is not None:
        ...
        print(f"INFERENCE mode â€” loading models from {models_dir}", flush=True)
        ...
        print("building lik-PF (train)â€¦", flush=True)
        likpf_train = build_likpf(train_wids, "train")
        print("building features (train)â€¦", flush=True)
...
# ## 6. Final ensemble
# 
# ### 6.1 Weighted blend of Track A and Track B
# `sp45_projection_submission.csv` (Track A) and `submission.csv` (Track B, "fleongg") are merged and combined with a fixed weight (`0.60` toward the sp45/particle-filter side by default), chosen from a small grid (`_BLEND_WEIGHTS_SP45`) validated by leaderboard/CV transfer rather than re-optimized in-notebook.
...
# %% [markdown]
# ### 6.2 Overlap-well safety guard
# 
# For the same train/test well-ID overlaps used in §4.2, this cell adds a guard rather than blindly trusting the lookup: it only overrides a well's blended prediction with the contacts-based reconstruction **if** that reconstruction backtests to under 1 ft RMSE against the well's own known (visible) prefix, and only for rows whose MD falls inside the train copy's covered range. Wells that don't pass the check keep the plain blend instead — a defensive measure against the train/test copies of an "overlap" well not being perfectly row-aligned. *(Worth double-checking this reuse of overlapping well IDs against the competition's data-usage rules before relying on it for a final submission.)*
...
# %% [markdown]
# ### 6.3 Submission integrity audit
# 
# A read-only sanity check: validates `submission.csv` has the right columns/row count/ID order against `sample_submission.csv`, checks all values are finite, and records a SHA-256 hash + summary stats to `version2_submission_audit.json` for reproducibility — this cell does not change any predictions.
...
# %% [markdown]
# ### 6.4 Visible-prefix calibration overlay (final layer)
# 
# The last refinement layer. For each well, it backtests several geology/particle-filter candidate trackers against the **already-known** prefix of that same well's trajectory (the part with `TVT_input` populated), and only nudges the final blended prediction toward a candidate when that backtest shows a consistent, large-enough improvement (thresholds set by a `conservative` / `balanced` / `aggressive` profile — this run uses `conservative`, selected in the very first cell). Because it only ever acts on evidence visible from each individual well's own known section, and always keeps the public blend as the fallback anchor, it behaves as a per-well confidence-gated calibration rather than a blanket rule change.
...
if not _GOLD_ENABLE:
    ...
    _chosen_path = _GOLD_WORK / 'submission_gold_prefix_balanced.csv'
...
# 
# Two things worth *seeing* rather than reading: how much each layer of the pipeline actually contributes to the final number, and where this run's tuning moved relative to the earlier public baseline (`rogii-lb7295-public-rebuild`).

# %% [markdown]
# ### 7.1 How the final prediction is assembled
# 
# The blend weights are fixed constants set earlier in the notebook (`_SELECTED_SP45_WEIGHT`, `PP.w_sub1`, `PP.w_pf`) — this just makes the composition visible instead of leaving it buried in code.
...
# 1) Track-level blend (Track A = sp45/particle-filter path, Track B = fleongg)
axes[0].pie(
    [_SELECTED_SP45_WEIGHT, 1 - _SELECTED_SP45_WEIGHT],
    labels=['Track A (sp45)', 'Track B (fleongg)'],
    autopct='%1.0f%%', colors=['#4C72B0', '#DD8452'], startangle=90,
)
axes[0].set_title('§6.1 final blend')

# 2) Within Track A: learned model+PF-prior sub1 vs the 128-seed lik-PF
axes[1].pie(
    [PP.w_sub1, 1 - PP.w_sub1],
    labels=['learned model + PF prior', '128-seed lik-PF'],
    autopct='%1.0f%%', colors=['#55A868', '#C44E52'], startangle=90,
)
axes[1].set_title('Track A internals (PP.w_sub1)')

# 3) Within sub1: GBM-learned delta vs the single-hypothesis particle filter
axes[2].pie(
    [1 - PP.w_pf, PP.w_pf],
    labels=['GBM-learned delta', 'single PF'],
    autopct='%1.0f%%', colors=['#8172B2', '#937860'], startangle=90,
)
axes[2].set_title('sub1 internals (PP.w_pf)')

plt.suptitle('Composition of the final TVT prediction', y=1.05)
plt.tight_layout()
plt.show()


# %% [markdown]
# ### 7.2 Ridge stacking weights — which base model does the stack trust?
# 
# The meta-learner's coefficients show how much weight each LightGBM/CatBoost fold ends up getting in the final stack — a quick, honest substitute for per-feature importance at the ensemble level. Runs gracefully to a message if the fitted `ridge_trainer` object doesn't expose `.estimator` under this `koolbox` version.
...
try:
    ridge_est = getattr(ridge_trainer, 'estimator', ridge_trainer)
    coefs = pd.Series(ridge_est.coef_, index=oof_preds.columns).sort_values()
    fig, ax = plt.subplots(figsize=(7, 0.45 * len(coefs) + 1))
    ax.barh(coefs.index, coefs.values, color=['#C44E52' if c < 0 else '#4C72B0' for c in coefs.values])
    ax.axvline(0, color='k', lw=.8)
    ax.set_title('Ridge stacking weight per base model')
    ax.set_xlabel('coefficient')
    plt.tight_layout()
    plt.show()
except Exception as e:
    print('Ridge coefficients unavailable in this environment (needs the fitted `ridge_trainer` from §4):', e)


# %% [markdown]
# ### 7.3 LightGBM feature importance
# 
# Reads `feature_importances_` off one of the trained/cached LightGBM boosters. Only runs when the model artifact and `features` list from §3/§4 are present in memory — this is intentionally *not* faked with placeholder numbers when they aren't.
...
try:
    lgb_artifacts = sorted(glob.glob(str(CFG.artifacts_path / 'models' / 'lightgbm-*' / '*.pkl')))
    if not lgb_artifacts:
        raise FileNotFoundError('no cached lightgbm-*.pkl artifact found under CFG.artifacts_path')
    fitted = joblib.load(lgb_artifacts[0])
    est = getattr(fitted, 'estimator', fitted)
    fi = pd.Series(est.feature_importances_, index=features).sort_values(ascending=False).head(20)
    fig, ax = plt.subplots(figsize=(7, 6))
    ax.barh(fi.index[::-1], fi.values[::-1], color='#4C72B0')
    ax.set_title('LightGBM (fold-1 config) — top 20 feature importances')
    plt.tight_layout()
    plt.show()
except Exception as e:
    print('Feature importances unavailable in this offline preview (needs a trained LightGBM artifact + `features` from §3/§4):', e)


# %% [markdown]
# <a id='8'></a>
# ## 8. What changed vs. the earlier public baseline (`rogii-lb7295-public-rebuild`)
# 
# Diffing this notebook against the earlier public build turns up one structural change and five retuned constants:
# 
# - **§4.2 selector → 3-way average.** The earlier build picked a single per-well tracker variant via `apply_selector_variant` (a lookup keyed on binned well statistics). This build instead **averages the three drift scales** (`pf_scale_3`, `pf_scale_5`, `pf_scale_8`) directly — trading the selector's occasional sharp-but-wrong pick for a smoother, lower-variance estimate. This is the one *structural* change; everything else below is retuning.
# - **§4.3 robust projection degree 4 → 3.** A lower-degree per-well fit is less prone to chasing local jitter into a wrong-branch excursion.
# - **§4.1/§5 `PP` constants retuned** (`alpha` 1.00→0.985, `tau` 85→120 ft, `w_pf` 0.09→0.05): a slightly damped global scale, a longer post-known-section warm-up, and less weight on the raw single-hypothesis PF now that the lik-PF is folded in as a feature.
# - **§6.1 blend weight** toward Track A (sp45) raised from 0.55 to 0.60.
# - **§6.4 gold-calibration `conservative` profile** loosened (`max_best` 9.0 ft → 12.0 ft), allowing slightly larger per-well corrections to qualify under the strictest profile.
# 
# The chart below turns the five numeric retunings into one picture (the selector→average change is qualitative and isn't a single number, so it's called out above instead).
...
changes = [
    ('PP.alpha', 1.000, 0.985),
    ('PP.tau (ft)', 85.0, 120.0),
    ('PP.w_pf', 0.09, 0.05),
    ('robfit degree', 4, 3),
    ('sp45 blend weight', 0.55, 0.60),
    ('gold conservative max_best (ft)', 9.0, 12.0),
]

labels = [c[0] for c in changes]
old_v = np.array([c[1] for c in changes], dtype=float)
new_v = np.array([c[2] for c in changes], dtype=float)
pct = (new_v - old_v) / np.abs(old_v) * 100

fig, ax = plt.subplots(figsize=(9, 4.5))
y = np.arange(len(labels))
ax.barh(y, pct, color=['#C44E52' if p < 0 else '#55A868' for p in pct])
ax.set_yticks(y)
ax.set_yticklabels(labels)
ax.axvline(0, color='k', lw=.8)
ax.set_xlabel('change vs. lb7295 baseline (%)')
ax.set_title('Retuned constants in this run')
for yi, p, o, n in zip(y, pct, old_v, new_v):
    ax.text(p + (1.5 if p >= 0 else -1.5), yi, f'{o:g} \u2192 {n:g}',
            va='center', ha='left' if p >= 0 else 'right', fontsize=9)
ax.set_xlim(min(pct.min(), 0) - 15, max(pct.max(), 0) + 15)
plt.tight_layout()
plt.show()


# %% [markdown]
# > **Note on "better."** These are the tuning deltas actually present in the diff, not a claim about leaderboard score — that depends on the private-test RMSE, which isn't something this notebook (or I) can verify offline. If you have the two runs' actual LB/CV numbers, it's worth dropping a one-line "CV: X → Y" callout right above this chart so readers see the real payoff, not just the direction of the tuning.

# %% [markdown]
# <a id='9'></a>
# ## 9. Takeaways
# 
# - **Don't fight the physics.** A gamma-ray particle filter anchored to the type well captures the actual geosteering signal ("which layer am I in") far better than a tabular model extrapolating blindly past the last known point — the GBM stack is there to learn the *residual* drift on top of it, not to replace it.
# - **Two independently-built trackers beat one bigger one.** Track A and Track B share the same physical idea but differ in implementation details (particle counts, beam search configs, feature sets), which decorrelates their errors enough that a simple weighted blend outperforms either alone.
# - **Calibrate per well, not globally.** The final overlay backtests each well's own known prefix before trusting a candidate correction — a small amount of local validation signal, used conservatively, is safer than a single global blend weight.
# - **Guard every shortcut.** The train/test well-ID overlap is only used after it passes its own local RMSE check, and the final file is hash-audited before submission — cheap insurance against a quietly wrong file winning.

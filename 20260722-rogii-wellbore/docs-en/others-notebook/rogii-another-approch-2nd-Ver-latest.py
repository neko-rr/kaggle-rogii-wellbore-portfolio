# %% [markdown]
# # A31 — Mean-Preserving Toe Tilt on the Qualified PF Branch
# 
# > **Attribution and scope**  
# > This notebook is a derived experiment based on the uploaded public 6.451 notebook. The upstream public pipeline is retained unchanged. The only new modeling element is the A31 postprocessor below.
# 
# ## Added essence
# 
# The original PF seed-branch hedge applies one constant offset across every qualified evaluation row. A31 preserves that offset **on average**, but redistributes a small part of it smoothly from Heel to Toe:
# 
# - slightly weaker correction near the beginning of the unknown interval;
# - slightly stronger correction near the Toe;
# - exactly zero mean incremental move on the target well;
# - no change to any non-target well;
# - hard fingerprint checks before writing the final submission.
# 
# This tests one physical hypothesis only: branch-offset uncertainty tends to accumulate with distance along the horizontal section.
# 
...
# %% cell 54
# A31 original layer: mean-preserving Heel-to-Toe tilt on the single qualified PF branch.
# Upstream public pipeline is unchanged. This is the only new prediction logic.
import hashlib as _a31_hashlib
import json as _a31_json
from pathlib import Path as _A31Path
import numpy as _a31_np
import pandas as _a31_pd

_A31_WORK = _A31Path('/kaggle/working') if _A31Path('/kaggle/working').exists() else _A31Path('.')
_A31_SUB = _A31_WORK / 'submission.csv'
_A31_SAMPLE = _A31Path(getattr(CFG, 'DATA', getattr(CFG, 'dataset_path', '/kaggle/input/competitions/rogii-wellbore-geology-prediction'))) / 'sample_submission.csv'
_A31_BRANCH_REPORT = _A31_WORK / 'pf_seed_branch_hedge_report.csv'
_A31_NAMED = _A31_WORK / 'submission_a31_mean_preserving_toe_tilt.csv'
_A31_REPORT = _A31_WORK / 'a31_mean_preserving_toe_tilt_report.json'

# Fingerprints of the actual uploaded source notebook's final prediction.
_A31_EXPECTED_ROWS = 14151
_A31_EXPECTED_SOURCE_FILE_SHA = 'b192d3f348ae00680dc4df942b95cef5fd708c636a741f77dfb6b6e89b9ded4a'
_A31_EXPECTED_SOURCE_PRED_SHA = '5e7b6d65f54498ffc2d071b705385da11e5aa2e6c3a4d16d4a2d7439878ba328'
_A31_EXPECTED_TARGET_WELL = '00e12e8b'
_A31_EXPECTED_TARGET_ROWS = 4301
_A31_EXPECTED_BRANCH_SHIFT_FT = 2.0

# One experimental lever only.
_A31_TILT_CAP_FT = 0.18


def _a31_file_sha(path):
    h = _a31_hashlib.sha256()
    with open(path, 'rb') as f:
        for chunk in iter(lambda: f.read(1 << 20), b''):
            h.update(chunk)
    return h.hexdigest()
...
def _a31_pred_sha(values):
    arr = _a31_np.asarray(values, dtype='<f8')
    return _a31_hashlib.sha256(arr.tobytes(order='C')).hexdigest()


def _a31_read_submission(path, label):
    if not path.exists():
        raise RuntimeError(f'A31 missing {label}: {path}')
    df = _a31_pd.read_csv(path)
    if list(df.columns) != ['id', 'tvt']:
        raise RuntimeError(f'A31 unexpected columns in {label}: {list(df.columns)}')
    df['id'] = df['id'].astype(str)
    df['tvt'] = _a31_pd.to_numeric(df['tvt'], errors='coerce').astype(float)
    if len(df) != _A31_EXPECTED_ROWS:
        raise RuntimeError(f'A31 unexpected row count in {label}: {len(df)}')
    if df['id'].duplicated().any() or not _a31_np.isfinite(df['tvt'].to_numpy()).all():
        raise RuntimeError(f'A31 invalid ids or tvt values in {label}')
    return df


_base = _a31_read_submission(_A31_SUB, 'source submission')
_sample = _a31_pd.read_csv(_A31_SAMPLE)
_sample['id'] = _sample['id'].astype(str)
if not _base['id'].equals(_sample['id']):
    raise RuntimeError('A31 source id order does not match sample_submission.csv')

_source_file_sha = _a31_file_sha(_A31_SUB)
_source_pred_sha = _a31_pred_sha(_base['tvt'].to_numpy(dtype=float))
if _source_file_sha != _A31_EXPECTED_SOURCE_FILE_SHA:
    raise RuntimeError(f'A31 unexpected source file SHA: {_source_file_sha}')
if _source_pred_sha != _A31_EXPECTED_SOURCE_PRED_SHA:
    raise RuntimeError(f'A31 unexpected source prediction SHA: {_source_pred_sha}')

if not _A31_BRANCH_REPORT.exists():
    raise RuntimeError('A31 branch hedge report is missing')
_br = _a31_pd.read_csv(_A31_BRANCH_REPORT)
if not {'well', 'reason', 'shift', 'moved_rows'}.issubset(_br.columns):
    raise RuntimeError(f'A31 branch report columns are incomplete: {list(_br.columns)}')
_applied = _br.loc[_br['reason'].astype(str).eq('applied')].copy()
if len(_applied) != 1:
    raise RuntimeError(f'A31 expected exactly one applied branch well, found {len(_applied)}')
_target_well = str(_applied.iloc[0]['well'])
_branch_shift = float(_applied.iloc[0]['shift'])
_reported_rows = int(_applied.iloc[0]['moved_rows'])
if _target_well != _A31_EXPECTED_TARGET_WELL:
    raise RuntimeError(f'A31 unexpected applied well: {_target_well}')
if abs(_branch_shift - _A31_EXPECTED_BRANCH_SHIFT_FT) > 1e-12:
    raise RuntimeError(f'A31 unexpected branch shift: {_branch_shift}')
if _reported_rows != _A31_EXPECTED_TARGET_ROWS:
    raise RuntimeError(f'A31 unexpected branch moved_rows: {_reported_rows}')

_parts = _base['id'].str.rsplit('_', n=1, expand=True)
_well = _parts[0].astype(str)
_row = _a31_pd.to_numeric(_parts[1], errors='raise').astype(int)
_stats = (globals().get('PF_SEED_BRANCH_STATS', {}) or {}).get(_target_well, {})
_eval_rows = set(int(x) for x in _stats.get('eval_rows', []))
_mask = _well.eq(_target_well).to_numpy()
if _eval_rows:
    _mask &= _row.isin(_eval_rows).to_numpy()
if int(_mask.sum()) != _A31_EXPECTED_TARGET_ROWS:
    raise RuntimeError(f'A31 target row count mismatch: {int(_mask.sum())}')

_target_idx = _a31_np.flatnonzero(_mask)
_order = _a31_np.argsort(_row.to_numpy()[_target_idx])
_ordered_idx = _target_idx[_order]
_ordered_rows = _row.to_numpy()[_ordered_idx].astype(float)
if len(_ordered_rows) < 2 or float(_ordered_rows[-1] - _ordered_rows[0]) <= 0.0:
    raise RuntimeError('A31 cannot construct target progress coordinate')

# Smoothstep progress: 0 at Heel, 1 at Toe.
_p = (_ordered_rows - _ordered_rows[0]) / (_ordered_rows[-1] - _ordered_rows[0])
_smooth_progress = _p * _p * (3.0 - 2.0 * _p)
_shape = 2.0 * _smooth_progress - 1.0
_shape -= float(_shape.mean())
_shape_scale = float(_a31_np.max(_a31_np.abs(_shape)))
if not _a31_np.isfinite(_shape_scale) or _shape_scale <= 0.0:
    raise RuntimeError('A31 invalid Toe-tilt shape')
_shape /= _shape_scale

# This is the single added essence: a small zero-mean tilt around the existing branch hedge.
_local_move = _A31_TILT_CAP_FT * _shape
_local_move -= float(_local_move.mean())
# Final rescale protects the cap after floating-point recentering.
_peak = float(_a31_np.max(_a31_np.abs(_local_move)))
if _peak > _A31_TILT_CAP_FT:
    _local_move *= _A31_TILT_CAP_FT / _peak
_local_move -= float(_local_move.mean())

if abs(float(_local_move.mean())) > 1e-12:
    raise RuntimeError(f'A31 target mean move is not zero: {float(_local_move.mean())}')
if float(_a31_np.max(_a31_np.abs(_local_move))) > _A31_TILT_CAP_FT + 1e-12:
    raise RuntimeError('A31 tilt cap violation')
if not (float(_local_move[0]) < 0.0 and float(_local_move[-1]) > 0.0):
    raise RuntimeError('A31 tilt direction is not Heel-negative / Toe-positive')

_before = _base['tvt'].to_numpy(dtype=float).copy()
_after = _before.copy()
_after[_ordered_idx] += _local_move
if not _a31_np.isfinite(_after).all():
    raise RuntimeError('A31 produced non-finite predictions')
_delta = _after - _before
if float(_a31_np.max(_a31_np.abs(_delta[~_mask]))) != 0.0:
    raise RuntimeError('A31 changed a non-target row')
if abs(float(_delta[_mask].mean())) > 1e-12:
    raise RuntimeError('A31 did not preserve the target-well mean')

_final = _base.copy()
_final['tvt'] = _after
_final.to_csv(_A31_WORK / 'submission_before_a31.csv', index=False)
_final.to_csv(_A31_NAMED, index=False)
_final.to_csv(_A31_SUB, index=False)

_report = {
    'experiment': 'A31',
    'strategy': 'mean-preserving smooth Heel-to-Toe tilt on the single applied PF branch hedge',
    'source_public_label': '6.451 uploaded notebook',
    'source_file_sha': _source_file_sha,
    'source_prediction_sha': _source_pred_sha,
    'target_well': _target_well,
    'target_rows': int(_mask.sum()),
    'original_branch_shift_ft': _branch_shift,
    'tilt_cap_ft': float(_A31_TILT_CAP_FT),
    'heel_increment_ft': float(_local_move[0]),
    'toe_increment_ft': float(_local_move[-1]),
    'target_mean_increment_ft': float(_local_move.mean()),
    'target_p95_abs_increment_ft': float(_a31_np.quantile(_a31_np.abs(_local_move), 0.95)),
    'target_max_abs_increment_ft': float(_a31_np.max(_a31_np.abs(_local_move))),
    'non_target_max_abs_increment_ft': float(_a31_np.max(_a31_np.abs(_delta[~_mask]))),
    'changed_rows': int(_a31_np.count_nonzero(_delta)),
    'named_output': str(_A31_NAMED),
    'final_file_sha': _a31_file_sha(_A31_SUB),
    'final_prediction_sha': _a31_pred_sha(_after),
}
with open(_A31_REPORT, 'w', encoding='utf-8') as f:
    _a31_json.dump(_report, f, indent=2, sort_keys=True)
print('A31 mean-preserving Toe tilt:', _a31_json.dumps(_report, indent=2, sort_keys=True), flush=True)
...
# %% cell 56
# A31 compact run summary for copying back to GPT.
import hashlib as _a31s_hashlib
import json as _a31s_json
from pathlib import Path as _A31SPath
import numpy as _a31s_np
import pandas as _a31s_pd

_A31S_WORK = _A31SPath('/kaggle/working') if _A31SPath('/kaggle/working').exists() else _A31SPath('.')
_A31S_SUB = _A31S_WORK / 'submission.csv'
_A31S_REPORT = _A31S_WORK / 'a31_mean_preserving_toe_tilt_report.json'
with open(_A31S_REPORT, 'r', encoding='utf-8') as f:
    _r = _a31s_json.load(f)
_df = _a31s_pd.read_csv(_A31S_SUB)
_file_sha = _a31s_hashlib.sha256(_A31S_SUB.read_bytes()).hexdigest()
_pred_sha = _a31s_hashlib.sha256(_a31s_np.asarray(_df['tvt'], dtype='<f8').tobytes()).hexdigest()

print('===== COPY THIS TO GPT START =====')
print('# A31 Run Summary')
for _k in [
    'experiment', 'strategy', 'source_public_label', 'source_file_sha',
    'source_prediction_sha', 'target_well', 'target_rows',
    'original_branch_shift_ft', 'tilt_cap_ft', 'heel_increment_ft',
    'toe_increment_ft', 'target_mean_increment_ft',
    'target_p95_abs_increment_ft', 'target_max_abs_increment_ft',
    'non_target_max_abs_increment_ft', 'changed_rows', 'named_output'
]:
    print(f'- {_k}: `{_r.get(_k)}`')
print(f'- rows: `{len(_df)}`')
print(f'- finite_tvt: `{bool(_a31s_np.isfinite(_df["tvt"].to_numpy(dtype=float)).all())}`')
print(f'- final_file_sha: `{_file_sha}`')
print(f'- final_prediction_sha: `{_pred_sha}`')
print(f'- final_tvt_min: `{float(_df["tvt"].min())}`')
print(f'- final_tvt_max: `{float(_df["tvt"].max())}`')
print(f'- final_tvt_mean: `{float(_df["tvt"].mean())}`')
print(f'- final_tvt_std: `{float(_df["tvt"].std(ddof=0))}`')

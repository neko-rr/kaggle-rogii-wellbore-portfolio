...
# --- cell 40 ---
# === MPKG-GATE PORT (kevinermille EXP-086 lineage; recon #4 docs/zh/09 §十二) ===
# Row-adaptive gated blend toward the pilkwang model-package predictions:
#   gate = gmax / (1 + (|pkg - base| / 6.0)^2);  final = (1-gate)*base + gate*pkg
# Published dose curve on his 7.10 base: gmax 0.005 -> 7.155 / 0.010 -> 7.077 / 0.015 -> target <7.053.
# Runs BEFORE the midpoint-hedge cell (hedge stays applied after, per the port plan).
# Correction cell body is kevinermille's cell-52 verbatim below the config block.
RUN_MODEL_PACKAGE_CORRECTION = True
MODEL_PACKAGE_GATED_MAX_WEIGHT = 0.01
MODEL_PACKAGE_GATED_SCALE = 6.0
MODEL_PACKAGE_GATED_CANDIDATES = (0.005, 0.010, 0.015)
MODEL_PACKAGE_ALLOW_AUTO_SEARCH = True
MODEL_PACKAGE_ROOTS = ['/kaggle/input/datasets/pilkwang/rogii-model-package',
                       '/kaggle/input/pilkwang/rogii-model-package',
                       '/kaggle/input/rogii-model-package']
# CFG is REDEFINED mid-notebook (dual-track merge: cell-3 CFG.dataset_path -> cell-23 CFG.DATA),
# so resolve the competition root robustly the same way kevinermille's A023 guard does.
from pathlib import Path as _MPCfgPath
_mp_root_candidates = []
_mp_cfg = globals().get('CFG')
for _mp_attr in ('dataset_path', 'DATA'):
    if _mp_cfg is not None and hasattr(_mp_cfg, _mp_attr):
        _mp_root_candidates.append(_MPCfgPath(getattr(_mp_cfg, _mp_attr)))
_mp_root_candidates += [_MPCfgPath('/kaggle/input/competitions/rogii-wellbore-geology-prediction'),
                        _MPCfgPath('/kaggle/input/rogii-wellbore-geology-prediction')]
_MP_DATA_ROOT = next(p for p in _mp_root_candidates if (p / 'sample_submission.csv').exists())
COMPETITION_DATA_ROOT = str(_MP_DATA_ROOT)
DATA_DIR = _MP_DATA_ROOT
SAMPLE_SUBMISSION = _MP_DATA_ROOT / 'sample_submission.csv'
...
if not bool(globals().get('RUN_MODEL_PACKAGE_CORRECTION', False)):
    ...
    def _mp_predict_torch_tcn(payload: dict, frame: _mp_pd.DataFrame, columns: list[str], entry: dict) -> _mp_np.ndarray:
        ...
        device = torch.device('cpu')  # EXP-085/087: force model-package TCN to CPU; avoids Kaggle CUDA no-kernel-image
...
_MH_ALPHA, _MH_MINMASS, _MH_SEPLO, _MH_SEPHI, _MH_CAP = 1.2, 0.22, 4.0, 40.0, 4.0
try:

# Extracted from rogii-geoanchor (rogii-geoanchor.ipynb)
# For analysis only — check license-ledger before adopting.

# --- cell 6 ---
# Select the active mathematical path.
SUBMISSION_PROFILE = 'dual_champion_suffix_arbiter'

PROFILE_PRESETS = {
    'dual_track_prefix_balanced': dict(
        sp45_blend_weight=0.55,
        run_model_package_correction=False,
    ),
    'dual_track_prefix_balanced_w054': dict(
        sp45_blend_weight=0.54,
        run_model_package_correction=False,
    ),
    'dual_track_prefix_modelpkg_005': dict(
        sp45_blend_weight=0.55,
    ...
    'dual_track_prefix_modelpkg_010': dict(
        sp45_blend_weight=0.55,
        run_model_package_correction=True,
        model_package_require=True,
    ...
    'dual_champion_suffix_arbiter': dict(
        sp45_blend_weight=0.55,
        # Use the saved-model package when mounted, but never make the
        # submission depend on that optional dataset.
        run_model_package_correction=True,
        model_package_require=False,
        model_package_gated_max_weight=0.005,
        model_package_gated_scale=6.0,
        run_suffix_arbiter=True,
        suffix_arbiter_profile='balanced',
    ),
}
...
# Data and mounted artifact roots.
...
# Residual ensemble and selector anchor.
RIDGE_PP_ALPHA = 1.0
RIDGE_PP_TAU = 85.0
RIDGE_PP_W_PF = 0.15
...
SP45_PROJECTION_DEGREE = 4
SP45_PROJECTION_BLEND_WEIGHT = 0.75

# Optional diagnostics; disabled for submission execution.
RUN_CV_REPORT = False
...
# Optional bimodal datum detector. Implemented, but not allowed to alter these submissions.
...
# Heel calibration supports the ambiguity diagnostics without changing the base path.
...
GUARDED_OVERRIDE_REF_COLS = ('EGFDU',)
...
SP45_BLEND_CANDIDATE_WEIGHTS = tuple(sorted(set((0.52, 0.54, 0.55, 0.56, 0.58, SP45_BLEND_WEIGHT))))

# Visible-prefix pseudo-holdout and bounded final move.
...
VISIBLE_PREFIX_BALANCED_MIN_GAIN = 0.55
VISIBLE_PREFIX_BALANCED_MAX_BEST = 12.0
VISIBLE_PREFIX_BALANCED_MIN_CONSISTENCY = 0.50
VISIBLE_PREFIX_BALANCED_BASE = 0.08
VISIBLE_PREFIX_BALANCED_GAIN_SCALE = 0.20
VISIBLE_PREFIX_BALANCED_MARGIN_SCALE = 0.06
VISIBLE_PREFIX_BALANCED_QUALITY_BONUS = 0.04
VISIBLE_PREFIX_BALANCED_CAP = 0.40
VISIBLE_PREFIX_BALANCED_CLIP_BASE = 10.0
VISIBLE_PREFIX_BALANCED_CLIP_GAIN = 4.5
VISIBLE_PREFIX_BALANCED_CLIP_MAX = 30.0
VISIBLE_PREFIX_BALANCED_DELTA_SOFT = 30.0
VISIBLE_PREFIX_BALANCED_P95_HARD = 75.0
...
# Optional saved-model residual correction, applied after visible-prefix calibration.
...
MODEL_PACKAGE_REQUIRE = bool(_profile.get('model_package_require', False))
MODEL_PACKAGE_ALLOW_AUTO_SEARCH = True
...
MODEL_PACKAGE_GATED_CANDIDATES = (0.005, 0.010)
MODEL_PACKAGE_DIFF_P95_DISABLE = None

# Dual-champion hidden-suffix arbiter. It never edits the known prefix and
# automatically rolls back a well when the physical/model evidence is weak.
RUN_DUAL_CHAMPION_SUFFIX_ARBITER = bool(_profile.get('run_suffix_arbiter', False))
SUFFIX_ARBITER_PROFILE = str(_profile.get('suffix_arbiter_profile', 'balanced'))
SUFFIX_ARBITER_PROFILE_ALPHA = {'conservative': 0.45, 'balanced': 0.70, 'strong': 0.90}
SUFFIX_ARBITER_PROFILE_MAX_MOVE = {'conservative': 6.0, 'balanced': 10.0, 'strong': 14.0}
SUFFIX_ARBITER_RAMP_ROWS = 140
SUFFIX_ARBITER_SMOOTH_WINDOW = 61
SUFFIX_ARBITER_MIN_SIGN_AGREEMENT = 0.62
SUFFIX_ARBITER_GR_Z_START = 1.35
SUFFIX_ARBITER_GR_Z_WIDTH = 3.0
SUFFIX_ARBITER_MAX_STEP = 0.30
SUFFIX_ARBITER_GLOBAL_MEAN_ABS_LIMIT = 4.5
SUFFIX_ARBITER_GLOBAL_P99_LIMIT = 10.0
SUFFIX_ARBITER_FORMATIONS = ('ANCC', 'ASTNU', 'ASTNL', 'EGFDU', 'EGFDL', 'BUDA')

print('submission_profile:', SUBMISSION_PROFILE)
print('residual_controls:', dict(alpha=RIDGE_PP_ALPHA, tau=RIDGE_PP_TAU, w_pf=RIDGE_PP_W_PF))
print('anchor_controls:', dict(ridge=SP45_RIDGE_MODEL_WEIGHT, selector=SP45_SELECTOR_WEIGHT, projection_degree=SP45_PROJECTION_DEGREE, projection_blend=SP45_PROJECTION_BLEND_WEIGHT))
print('dual_track_sp45_weight:', SP45_BLEND_WEIGHT)
print('visible_prefix_controls:', dict(profile=VISIBLE_PREFIX_PROFILE, cap=VISIBLE_PREFIX_BALANCED_CAP, clip_max=VISIBLE_PREFIX_BALANCED_CLIP_MAX, cuts=VISIBLE_PREFIX_CUT_FRACS))
print('model_package_controls:', dict(enabled=RUN_MODEL_PACKAGE_CORRECTION, required=MODEL_PACKAGE_REQUIRE, max_weight=MODEL_PACKAGE_GATED_MAX_WEIGHT, scale=MODEL_PACKAGE_GATED_SCALE))
print('suffix_arbiter_controls:', dict(enabled=RUN_DUAL_CHAMPION_SUFFIX_ARBITER, profile=SUFFIX_ARBITER_PROFILE, alpha=SUFFIX_ARBITER_PROFILE_ALPHA.get(SUFFIX_ARBITER_PROFILE), max_move=SUFFIX_ARBITER_PROFILE_MAX_MOVE.get(SUFFIX_ARBITER_PROFILE)))
...
pp_params = {
    'alpha': float(RIDGE_PP_ALPHA),
    'tau': float(RIDGE_PP_TAU),
    'w_pf': float(RIDGE_PP_W_PF),
}
...
_GOLD_PROFILES = {
    ...
    'balanced':     dict(min_gain=VISIBLE_PREFIX_BALANCED_MIN_GAIN, max_best=VISIBLE_PREFIX_BALANCED_MAX_BEST, min_consistency=VISIBLE_PREFIX_BALANCED_MIN_CONSISTENCY, base=VISIBLE_PREFIX_BALANCED_BASE, gain_scale=VISIBLE_PREFIX_BALANCED_GAIN_SCALE, margin_scale=VISIBLE_PREFIX_BALANCED_MARGIN_SCALE, quality_bonus=VISIBLE_PREFIX_BALANCED_QUALITY_BONUS, cap=VISIBLE_PREFIX_BALANCED_CAP, clip_base=VISIBLE_PREFIX_BALANCED_CLIP_BASE, clip_gain=VISIBLE_PREFIX_BALANCED_CLIP_GAIN, clip_max=VISIBLE_PREFIX_BALANCED_CLIP_MAX, delta_soft=VISIBLE_PREFIX_BALANCED_DELTA_SOFT, p95_hard=VISIBLE_PREFIX_BALANCED_P95_HARD),
...
if not bool(globals().get('RUN_MODEL_PACKAGE_CORRECTION', False)):
    ...
    _mp_optional_error = None
    try:
        _mp_pkg_sub, _mp_pred_report, _mp_weight_report, _mp_info = _mp_build_submission()
    except Exception as _mp_exc:
        if bool(globals().get('MODEL_PACKAGE_REQUIRE', False)):
            raise
        _mp_optional_error = f'{type(_mp_exc).__name__}: {_mp_exc}'
        _mp_pkg_sub = None
        _mp_pred_report = _mp_pd.DataFrame()
        _mp_weight_report = _mp_pd.DataFrame()
        _mp_info = {}
        print('🐈‍⬛ optional model package unavailable; keeping prefix champion:', _mp_optional_error, flush=True)

    if _mp_pkg_sub is None:
        # Deterministic rollback: preserve the already audited contact/prefix
        # champion and delete any stale package-only artifact from a rerun.
        _mp_base.to_csv(_mp_final_output, index=False)
        _stale_pkg = _mp_work / 'submission_model_package_only.csv'
        if _stale_pkg.exists():
            _stale_pkg.unlink()
        _mp_pd.Series({
            'model_package_available': False,
            'required': bool(globals().get('MODEL_PACKAGE_REQUIRE', False)),
            'fallback': 'prefix_champion_submission',
            'reason': _mp_optional_error or 'model package dataset was not found',
        }).to_csv(_mp_work / 'model_package_correction_summary.csv')
        globals()['FINAL_BASE_SOURCE_LABEL'] = 'prefix_champion_no_model_package'
        ...
        globals()['FINAL_MODEL_PACKAGE_AVAILABLE'] = False
        print('🐈‍⬛ model-package fallback OK; submission.csv preserved', _mp_base.shape, flush=True)
    else:
...
# --- cell 54 ---
# Dual-champion hidden-suffix correction.
# Runs after the optional 0.5%-gated model-package layer and before the final audit.
if not bool(globals().get('RUN_DUAL_CHAMPION_SUFFIX_ARBITER', False)):
    print('🐈‍⬛ suffix arbiter disabled; keeping champion submission.csv', flush=True)
else:
    import hashlib as _sa_hashlib
    import json as _sa_json
    import glob as _sa_glob
    from pathlib import Path as _SaPath

    import numpy as _sa_np
    import pandas as _sa_pd

    print('🐈‍⬛ DUAL-CHAMPION SUFFIX ARBITER', flush=True)

    def _sa_work_dir():
        explicit = globals().get('OUTPUT_DIR', None)
        if explicit is not None:
            p = _SaPath(explicit)
            p.mkdir(parents=True, exist_ok=True)
            return p
        p = _SaPath('/kaggle/working')
        return p if p.exists() else _SaPath('.')

    def _sa_find_data():
        roots = []
        for value in (
            globals().get('COMPETITION_DATA_ROOT', None),
            globals().get('DATA_DIR', None),
        ):
            if value:
                roots.append(_SaPath(value))
        roots.extend([
            _SaPath('/kaggle/input/competitions/rogii-wellbore-geology-prediction'),
            _SaPath('/kaggle/input/rogii-wellbore-geology-prediction'),
        ])
        for root in roots:
            if (root / 'train').exists() and (root / 'test').exists() and (root / 'sample_submission.csv').exists():
                return root
        for path in _sa_glob.glob('/kaggle/input/**/sample_submission.csv', recursive=True):
            root = _SaPath(path).parent
            if (root / 'train').exists() and (root / 'test').exists():
                return root
        raise RuntimeError('competition data root not found')

    def _sa_read(path, label, required=True):
        path = _SaPath(path)
        if not path.exists():
            if required:
                raise RuntimeError(f'missing {label}: {path}')
            return None
        frame = _sa_pd.read_csv(path)
        if not {'id', 'tvt'}.issubset(frame.columns):
            raise RuntimeError(f'{label} has invalid columns')
        frame = frame[['id', 'tvt']].copy()
        frame['id'] = frame['id'].astype(str)
        frame['tvt'] = _sa_pd.to_numeric(frame['tvt'], errors='coerce')
        if not _sa_np.isfinite(frame['tvt'].to_numpy(dtype=float)).all():
            raise RuntimeError(f'{label} contains non-finite predictions')
        return frame

    def _sa_join(base, other, name):
        if other is None:
            return base
        out = base.merge(other.rename(columns={'tvt': name}), on='id', how='left', validate='one_to_one')
        return out

    def _sa_interp(md_src, value_src, md_dst):
        md_src = _sa_np.asarray(md_src, dtype=float)
        value_src = _sa_np.asarray(value_src, dtype=float)
        md_dst = _sa_np.asarray(md_dst, dtype=float)
        good = _sa_np.isfinite(md_src) & _sa_np.isfinite(value_src)
        if int(good.sum()) < 3:
            return _sa_np.full(len(md_dst), _sa_np.nan, dtype=float)
        tmp = _sa_pd.DataFrame({'md': md_src[good], 'v': value_src[good]}).groupby('md', as_index=False)['v'].median().sort_values('md')
        x = tmp['md'].to_numpy(dtype=float)
        y = tmp['v'].to_numpy(dtype=float)
        if len(x) < 3:
            return _sa_np.full(len(md_dst), _sa_np.nan, dtype=float)
        return _sa_np.interp(md_dst, x, y, left=_sa_np.nan, right=_sa_np.nan)

    def _sa_smooth(values, window):
        values = _sa_np.asarray(values, dtype=float)
        if len(values) == 0:
            return values.copy()
        w = max(5, int(window))
        if w % 2 == 0:
            w += 1
        s = _sa_pd.Series(values)
        s = s.interpolate(limit_direction='both')
        s = s.rolling(w, center=True, min_periods=1).median()
        s = s.rolling(max(5, w // 3), center=True, min_periods=1).mean()
        return s.to_numpy(dtype=float)

    def _sa_boundary_center(values, n_head=36):
        values = _sa_np.asarray(values, dtype=float).copy()
        good = _sa_np.flatnonzero(_sa_np.isfinite(values))
        if len(good) == 0:
            return values
        head = good[:min(int(n_head), len(good))]
        center = float(_sa_np.nanmedian(values[head]))
        return values - center

    def _sa_slope_clamp(move, max_step):
        x = _sa_np.asarray(move, dtype=float).copy()
        if len(x) <= 1:
            return x
        step = float(max_step)
        for i in range(1, len(x)):
            x[i] = _sa_np.clip(x[i], x[i - 1] - step, x[i - 1] + step)
        for i in range(len(x) - 2, -1, -1):
            x[i] = _sa_np.clip(x[i], x[i + 1] - step, x[i + 1] + step)
        return x

    def _sa_sha(path):
        h = _sa_hashlib.sha256()
        with open(path, 'rb') as f:
            for chunk in iter(lambda: f.read(1 << 20), b''):
                h.update(chunk)
        return h.hexdigest()

    _SA_WORK = _sa_work_dir()
    _SA_DATA = _sa_find_data()
    _SA_SAMPLE = _sa_pd.read_csv(_SA_DATA / 'sample_submission.csv')[['id']].copy()
    _SA_SAMPLE['id'] = _SA_SAMPLE['id'].astype(str)

    # Anchor A: exact prefix-verified contact trajectory.
    _anchor_a = _sa_read(_SA_WORK / 'submission_self_verified_anchor.csv', 'contact champion', required=True)
    # Anchor B: current audited submission. It is model-package gated when available,
    # otherwise it is the exact prefix/contact champion.
    _anchor_b = _sa_read(_SA_WORK / 'submission.csv', 'current audited champion', required=True)
    _pkg_only = _sa_read(_SA_WORK / 'submission_model_package_only.csv', 'model-package raw path', required=False)
    _learned = _sa_read(_SA_WORK / 'learned_trajectory_submission.csv', 'learned path', required=False)
    _sp45 = _sa_read(_SA_WORK / 'sp45_projection_submission.csv', 'physical path', required=False)

    _frame = _SA_SAMPLE.merge(_anchor_a.rename(columns={'tvt': 'tvt_anchor_a'}), on='id', how='left', validate='one_to_one')
    _frame = _sa_join(_frame, _anchor_b, 'tvt_anchor_b')
    _frame = _sa_join(_frame, _pkg_only, 'tvt_pkg')
    _frame = _sa_join(_frame, _learned, 'tvt_learned')
    _frame = _sa_join(_frame, _sp45, 'tvt_sp45')
    required_cols = ['tvt_anchor_a', 'tvt_anchor_b']
    if _frame[required_cols].isna().any().any():
        raise RuntimeError('champion anchor id mismatch')
    _frame['well'] = _frame['id'].str[:8]
    _frame['row_idx'] = _frame['id'].str[9:].astype(int)
    _frame['_order'] = _sa_np.arange(len(_frame), dtype=int)

    _profile = str(globals().get('SUFFIX_ARBITER_PROFILE', 'balanced'))
    _alpha_map = dict(globals().get('SUFFIX_ARBITER_PROFILE_ALPHA', {'conservative': 0.45, 'balanced': 0.70, 'strong': 0.90}))
    _max_map = dict(globals().get('SUFFIX_ARBITER_PROFILE_MAX_MOVE', {'conservative': 6.0, 'balanced': 10.0, 'strong': 14.0}))
    if _profile not in _alpha_map or _profile not in _max_map:
        raise ValueError(f'unknown suffix arbiter profile: {_profile}')
    _selected_alpha = float(_alpha_map[_profile])
    _selected_max = float(_max_map[_profile])
    _ramp_rows = max(20, int(globals().get('SUFFIX_ARBITER_RAMP_ROWS', 140)))
    _smooth_window = max(11, int(globals().get('SUFFIX_ARBITER_SMOOTH_WINDOW', 61)))
    _min_agree = float(globals().get('SUFFIX_ARBITER_MIN_SIGN_AGREEMENT', 0.62))
    _gr_start = float(globals().get('SUFFIX_ARBITER_GR_Z_START', 1.35))
    _gr_width = max(0.25, float(globals().get('SUFFIX_ARBITER_GR_Z_WIDTH', 3.0)))
    _max_step = float(globals().get('SUFFIX_ARBITER_MAX_STEP', 0.30))
    _formations = tuple(globals().get('SUFFIX_ARBITER_FORMATIONS', ('ANCC', 'ASTNU', 'ASTNL', 'EGFDU', 'EGFDL', 'BUDA')))

    _candidate_moves = {name: _sa_np.zeros(len(_frame), dtype=float) for name in _alpha_map}
    _well_reports = []
    _source_rows = []

    for _wid, _g0 in _frame.groupby('well', sort=False):
        _g = _g0.sort_values('row_idx').copy()
        _positions = _g['_order'].to_numpy(dtype=int)
        try:
            _hw_te_path = _SA_DATA / 'test' / f'{_wid}__horizontal_well.csv'
            _hw_tr_path = _SA_DATA / 'train' / f'{_wid}__horizontal_well.csv'
            if not _hw_te_path.exists() or not _hw_tr_path.exists():
                raise RuntimeError('same-well train/test pair unavailable')
            _hw_te = _sa_pd.read_csv(_hw_te_path)
            _hw_tr = _sa_pd.read_csv(_hw_tr_path)
            for col in ('MD', 'Z'):
                if col not in _hw_te.columns or col not in _hw_tr.columns:
                    raise RuntimeError(f'missing trajectory column {col}')

            _md_te = _hw_te['MD'].to_numpy(dtype=float)
            _z_te = _hw_te['Z'].to_numpy(dtype=float)
            _md_tr = _hw_tr['MD'].to_numpy(dtype=float)
            _z_tr_i = _sa_interp(_md_tr, _hw_tr['Z'].to_numpy(dtype=float), _md_te)
            _known = _sa_np.isfinite(_sa_pd.to_numeric(_hw_te.get('TVT_input', _sa_pd.Series(_sa_np.nan, index=_hw_te.index)), errors='coerce').to_numpy(dtype=float))
            _hidden_all = _sa_np.flatnonzero(~_known)
            _first_hidden = int(_hidden_all[0]) if len(_hidden_all) else int(_g['row_idx'].min())
            _ri = _g['row_idx'].to_numpy(dtype=int)
            if int(_ri.min()) < 0 or int(_ri.max()) >= len(_hw_te):
                raise RuntimeError('submission row index outside test well')

            # Physical test-minus-train drift. For each formation:
            # dTVT = (surface_test - surface_train) - (Z_test - Z_train).
            _surface_full = []
            _surface_names = []
            _tail_known = _sa_np.flatnonzero(_known & (_sa_np.arange(len(_hw_te)) < _first_hidden))
            _tail_known = _tail_known[-min(240, len(_tail_known)):]
            for _fn in _formations:
                if _fn not in _hw_te.columns or _fn not in _hw_tr.columns:
                    continue
                _f_te = _sa_pd.to_numeric(_hw_te[_fn], errors='coerce').to_numpy(dtype=float)
                _f_tr_i = _sa_interp(_md_tr, _sa_pd.to_numeric(_hw_tr[_fn], errors='coerce').to_numpy(dtype=float), _md_te)
                _raw = (_f_te - _f_tr_i) - (_z_te - _z_tr_i)
                if int(_sa_np.isfinite(_raw).sum()) < max(80, len(_g)):
                    continue
                if len(_tail_known):
                    _center_vals = _raw[_tail_known]
                    _center_vals = _center_vals[_sa_np.isfinite(_center_vals)]
                else:
                    _center_vals = _raw[_sa_np.isfinite(_raw)][:50]
                if len(_center_vals) == 0:
                    continue
                _raw = _raw - float(_sa_np.nanmedian(_center_vals))
                _raw = _sa_smooth(_raw, _smooth_window)
                _surface_full.append(_raw)
                _surface_names.append(_fn)

            if _surface_full:
                _smat = _sa_np.vstack(_surface_full)
                _surface_cons_full = _sa_np.nanmedian(_smat, axis=0)
                _surface_spread_full = _sa_np.nanmedian(_sa_np.abs(_smat - _surface_cons_full[None, :]), axis=0)
                _surface = _surface_cons_full[_ri]
                _surface_spread = _surface_spread_full[_ri]
                _surface = _sa_boundary_center(_sa_smooth(_surface, _smooth_window), n_head=24)
                _surface_quality = 1.0 / (1.0 + (_sa_np.nan_to_num(_surface_spread, nan=9.0) / 2.5) ** 2)
            else:
                _surface = _sa_np.full(len(_g), _sa_np.nan, dtype=float)
                _surface_spread = _sa_np.full(len(_g), _sa_np.nan, dtype=float)
                _surface_quality = _sa_np.ones(len(_g), dtype=float)

            # GR novelty is calibrated only on the visible prefix of this same-well pair.
            _gr_gate = _sa_np.zeros(len(_g), dtype=float)
            _gr_z_rows = _sa_np.zeros(len(_g), dtype=float)
            if 'GR' in _hw_te.columns and 'GR' in _hw_tr.columns:
                _gr_te = _sa_pd.to_numeric(_hw_te['GR'], errors='coerce').to_numpy(dtype=float)
                _gr_tr_i = _sa_interp(_md_tr, _sa_pd.to_numeric(_hw_tr['GR'], errors='coerce').to_numpy(dtype=float), _md_te)
                _gd = _sa_np.abs(_gr_te - _gr_tr_i)
                _vis = _gd[_tail_known] if len(_tail_known) else _gd[_known]
                _vis = _vis[_sa_np.isfinite(_vis)]
                if len(_vis) >= 20:
                    _med = float(_sa_np.nanmedian(_vis))
                    _mad = float(_sa_np.nanmedian(_sa_np.abs(_vis - _med)))
                    _scale = max(2.0, 1.4826 * _mad)
                    _gz = (_gd - _med) / _scale
                    _gz = _sa_smooth(_gz, max(21, _smooth_window // 2))
                    _gr_z_rows = _sa_np.nan_to_num(_gz[_ri], nan=0.0, posinf=0.0, neginf=0.0)
                    _gr_gate = _sa_np.clip((_gr_z_rows - _gr_start) / _gr_width, 0.0, 1.0)

            _anchor_a_v = _g['tvt_anchor_a'].to_numpy(dtype=float)
            _sources = []
            _weights = []
            _source_names = []
            if _sa_np.isfinite(_surface).sum() == len(_surface):
                _sources.append(_sa_np.clip(_surface, -24.0, 24.0))
                _weights.append(0.48)
                _source_names.append('formation_drift')

            def _add_model_source(column, name, scale, weight, clip):
                if column not in _g.columns:
                    return
                arr = _g[column].to_numpy(dtype=float) - _anchor_a_v
                if not _sa_np.isfinite(arr).all():
                    return
                arr = _sa_boundary_center(arr, n_head=30)
                arr = _sa_smooth(arr, _smooth_window)
                arr = _sa_np.clip(float(scale) * arr, -float(clip), float(clip))
                _sources.append(arr)
                _weights.append(float(weight))
                _source_names.append(name)

            _add_model_source('tvt_pkg', 'model_package', 0.35, 0.24, 20.0)
            _add_model_source('tvt_learned', 'learned', 0.50, 0.17, 18.0)
            _add_model_source('tvt_sp45', 'physical_pf', 0.50, 0.11, 18.0)

            if len(_sources) < 2:
                raise RuntimeError(f'insufficient independent sources ({len(_sources)})')
            _mat = _sa_np.vstack(_sources)
            _w = _sa_np.asarray(_weights, dtype=float)
            _w = _w / _w.sum()
            _consensus = _sa_np.sum(_w[:, None] * _mat, axis=0)
            _mad = _sa_np.sum(_w[:, None] * _sa_np.abs(_mat - _consensus[None, :]), axis=0)
            _sign = _sa_np.sign(_consensus)
            _agree = _sa_np.sum(_w[:, None] * ((_sa_np.sign(_mat) == _sign[None, :]) | (_sa_np.abs(_mat) < 0.35)), axis=0)
            _agree_gate = _sa_np.clip((_agree - _min_agree) / max(1e-6, 1.0 - _min_agree), 0.0, 1.0)
            _disagreement_gate = 1.0 / (1.0 + (_mad / 4.5) ** 2)
            _surface_activity = _sa_np.where(_sa_np.isfinite(_surface), _sa_np.clip(_sa_np.abs(_surface) / 2.0, 0.0, 1.0), 0.0)
            _novelty_gate = _sa_np.maximum(_gr_gate, 0.85 * _surface_activity)
            _evidence_gate = _agree_gate * _disagreement_gate * _surface_quality

            _distance = _sa_np.maximum(0.0, _ri.astype(float) - float(_first_hidden))
            _ramp = 1.0 - _sa_np.exp(-_distance / float(_ramp_rows))
            _raw_move = _ramp * _novelty_gate * _evidence_gate * _consensus
            _raw_move = _sa_smooth(_raw_move, max(15, _smooth_window // 2))
            _raw_move = _sa_slope_clamp(_raw_move, _max_step)

            # Local confidence guards. Any failure keeps Anchor B for this well.
            _first_guard = float(_sa_np.max(_sa_np.abs(_raw_move[:min(30, len(_raw_move))]))) if len(_raw_move) else 0.0
            _active_frac = float(_sa_np.mean(_sa_np.abs(_raw_move) > 0.15)) if len(_raw_move) else 0.0
            _mean_agree = float(_sa_np.mean(_agree))
            _mean_evidence = float(_sa_np.mean(_evidence_gate))
            _well_ok = bool(
                _sa_np.isfinite(_raw_move).all()
                and _first_guard <= 1.25
                and _mean_agree >= 0.55
                and _mean_evidence >= 0.03
                and _active_frac <= 0.98
            )
            if not _well_ok:
                _raw_move[:] = 0.0
            else:
                # A low-signal well must truly remain on the champion path;
                # do not report rollback while still applying numerical noise.
                _selected_preview = _sa_np.clip(_selected_alpha * _raw_move, -_selected_max, _selected_max)
                if float(_sa_np.max(_sa_np.abs(_selected_preview))) <= 0.05:
                    _raw_move[:] = 0.0

            for _name, _a in _alpha_map.items():
                _cap = float(_max_map[_name])
                _move = _sa_np.clip(float(_a) * _raw_move, -_cap, _cap)
                _move = _sa_slope_clamp(_move, min(_max_step, 0.30))
                _candidate_moves[_name][_positions] = _move

            for _sn, _sv, _sw in zip(_source_names, _sources, _weights):
                _source_rows.append({
                    'well': _wid,
                    'source': _sn,
                    'weight': float(_sw),
                    'mean_signed_signal': float(_sa_np.mean(_sv)),
                    'mean_abs_signal': float(_sa_np.mean(_sa_np.abs(_sv))),
                    'p95_abs_signal': float(_sa_np.quantile(_sa_np.abs(_sv), 0.95)),
                })
            _sel_move = _sa_np.clip(_selected_alpha * _raw_move, -_selected_max, _selected_max)
            _well_reports.append({
                'well': _wid,
                'status': 'applied' if _well_ok and float(_sa_np.max(_sa_np.abs(_sel_move))) > 0.05 else ('kept_anchor_low_signal' if _well_ok else 'rollback_guard'),
                'rows': int(len(_g)),
                'first_hidden_row': int(_first_hidden),
                'surface_sources': ','.join(_surface_names),
                'independent_sources': int(len(_sources)),
                'mean_gr_novelty_z': float(_sa_np.mean(_gr_z_rows)),
                'p95_gr_novelty_z': float(_sa_np.quantile(_gr_z_rows, 0.95)),
                'mean_sign_agreement': _mean_agree,
                'mean_evidence_gate': _mean_evidence,
                'active_fraction': _active_frac,
                'mean_abs_move': float(_sa_np.mean(_sa_np.abs(_sel_move))),
                'p95_abs_move': float(_sa_np.quantile(_sa_np.abs(_sel_move), 0.95)),
                'max_abs_move': float(_sa_np.max(_sa_np.abs(_sel_move))),
                'continuity_guard_first30': _first_guard,
            })
            print('  %s | %s | sources=%d | agree=%.3f | mean|move|=%.3f | p95=%.3f | max=%.3f' % (
                _wid,
                _well_reports[-1]['status'],
                len(_sources),
                _mean_agree,
                _well_reports[-1]['mean_abs_move'],
                _well_reports[-1]['p95_abs_move'],
                _well_reports[-1]['max_abs_move'],
            ), flush=True)
        except Exception as _e:
            _well_reports.append({
                'well': _wid,
            ...
            print('  %s | rollback_error | %s' % (_wid, _e), flush=True)

    _base_v = _frame['tvt_anchor_b'].to_numpy(dtype=float)
    _profile_rows = []
    for _name in sorted(_alpha_map, key=lambda x: float(_alpha_map[x])):
        _move = _candidate_moves[_name]
        _pred = _base_v + _move
        _candidate = _frame[['id', '_order']].copy().sort_values('_order')
        _candidate['tvt'] = _pred[_candidate['_order'].to_numpy(dtype=int)]
        _candidate = _candidate[['id', 'tvt']]
        if len(_candidate) != len(_SA_SAMPLE) or not _candidate['id'].equals(_SA_SAMPLE['id']):
            raise RuntimeError(f'{_name} candidate id-order mismatch')
        if not _sa_np.isfinite(_candidate['tvt'].to_numpy(dtype=float)).all():
            raise RuntimeError(f'{_name} candidate contains non-finite values')
        _path = _SA_WORK / f'submission_dual_champion_{_name}.csv'
        _candidate.to_csv(_path, index=False)
        _profile_rows.append({
            'profile': _name,
            'selected': bool(_name == _profile),
            'alpha': float(_alpha_map[_name]),
            'max_move_cap': float(_max_map[_name]),
            'mean_abs_move': float(_sa_np.mean(_sa_np.abs(_move))),
            'p95_abs_move': float(_sa_np.quantile(_sa_np.abs(_move), 0.95)),
            'p99_abs_move': float(_sa_np.quantile(_sa_np.abs(_move), 0.99)),
            'max_abs_move': float(_sa_np.max(_sa_np.abs(_move))),
            'sha256': _sa_sha(_path),
            'file': str(_path),
        })

    _selected_move = _candidate_moves[_profile]
    _global_mean = float(_sa_np.mean(_sa_np.abs(_selected_move)))
    _global_p99 = float(_sa_np.quantile(_sa_np.abs(_selected_move), 0.99))
    _global_max = float(_sa_np.max(_sa_np.abs(_selected_move)))
    _mean_limit = float(globals().get('SUFFIX_ARBITER_GLOBAL_MEAN_ABS_LIMIT', 4.5))
    _p99_limit = min(float(globals().get('SUFFIX_ARBITER_GLOBAL_P99_LIMIT', 10.0)), _selected_max)
    _global_ok = bool(
        _sa_np.isfinite(_selected_move).all()
        and _global_mean <= _mean_limit
        and _global_p99 <= _p99_limit + 1e-9
        and _global_max <= _selected_max + 1e-9
    )

    _anchor_b.to_csv(_SA_WORK / 'submission_dual_champion_anchor.csv', index=False)
    if _global_ok:
        _chosen = _sa_pd.read_csv(_SA_WORK / f'submission_dual_champion_{_profile}.csv')
        _chosen.to_csv(_SA_WORK / 'submission.csv', index=False)
        _selection = f'dual_champion_{_profile}'
    else:
        _anchor_b.to_csv(_SA_WORK / 'submission.csv', index=False)
        _selection = 'rollback_to_current_champion'

    _well_report = _sa_pd.DataFrame(_well_reports)
    _well_report.to_csv(_SA_WORK / 'dual_champion_suffix_arbiter_report.csv', index=False)
    _sa_pd.DataFrame(_source_rows).to_csv(_SA_WORK / 'dual_champion_suffix_sources.csv', index=False)
    _profile_report = _sa_pd.DataFrame(_profile_rows)
    _profile_report.to_csv(_SA_WORK / 'dual_champion_profile_report.csv', index=False)

    _audit = {
        'selection': _selection,
        'selected_profile': _profile,
        'global_guard_passed': _global_ok,
        'rows': int(len(_SA_SAMPLE)),
        'wells': int(_frame['well'].nunique()),
        'applied_wells': int((_well_report.get('status') == 'applied').sum()) if 'status' in _well_report else 0,
        'mean_abs_move': _global_mean,
        'p99_abs_move': _global_p99,
        'max_abs_move': _global_max,
        'anchor_a_sha256': _sa_sha(_SA_WORK / 'submission_self_verified_anchor.csv'),
        'anchor_b_sha256': _sa_sha(_SA_WORK / 'submission_dual_champion_anchor.csv'),
        'final_sha256': _sa_sha(_SA_WORK / 'submission.csv'),
    }
    with open(_SA_WORK / 'dual_champion_suffix_arbiter_audit.json', 'w', encoding='utf-8') as f:
        _sa_json.dump(_audit, f, indent=2, sort_keys=True)
    print('🐈‍⬛ suffix arbiter audit:', _audit, flush=True)

# %% [markdown] cell 0
"""
# ROGII 6.594 Anchor + PF 1.3 Shape Residual

## Overview and Purpose
This notebook executes a highly controlled leaderboard experiment designed to test an interior response-curve hypothesis. Rather than overhauling the entire pipeline, it takes a reproducible 6.594 final level solution and keeps it largely unchanged. The single controlled modification is the restoration of a small amount of trajectory shape generated by a Particle Filter (PF) configuration utilizing a Gamma Ray (GR) noise scale of `gs * 1.3`. 

The discarded PF-1.3 candidate is utilized strictly as a shape signal. The specific experimental mechanics are as follows:
* Identifies the single target well (00e12e8b) where the PF seed-branch hedge is applied.
* Computes the residual between the `learned_w0.60` prediction and the `contact_before_branch_hedge`.
* Robustly smooths and centers this residual.
* Adds exactly 10% of the centered shape residual to the 6.594 prediction.
* Hard-limits the move to approximately ±0.40 ft to prevent catastrophic drift.
* Keeps the target-well mean move at zero to preserve the successful +2.0 ft branch level.
* Keeps all non-target wells bit-for-bit unchanged prior to CSV serialization.

## ROGII Contact-Gated Stratigraphic Alignment
The underlying architecture builds a target-free True Vertical Thickness (TVT) trajectory using three distinct signals: a Ridge/PF anchor, a learned trajectory branch, and contact-based self-verification on the visible prefix. Correction layers are strictly gated and are only permitted to modify the `submission.csv` when local diagnostics mathematically support the move.

### Prediction Flow
The pipeline applies logic ordered from broad statistical fallbacks to highly localized self-verification:
1. **Anchor Generation:** Builds a Ridge/PF artifact trajectory alongside a physical/PF selector trajectory.
2. **Projection:** Blends them into an SP45-style anchor and applies robust projection in normalized Measured Depth (MD) space.
3. **Learned Branch:** Constructs the learned trajectory branch from mounted pretrained boosters.
4. **Trajectory Blending:** Blends the projected anchor and the learned branch.
5. **Guarded Reconstruction:** Applies same-well contact reconstruction *only* when the visible prefix proves compatibility.
6. **Prefix Calibration:** Runs visible-prefix calibration as an audit or as a bounded final correction.
7. **Final Audit:** Audits the final `submission.csv` against `sample_submission.csv` to ensure structural integrity.

## Safety Guards
To ensure this remains a controlled experiment rather than a random perturbation, the notebook enforces strict execution guards:
* Requires the normalized 6.594 source prediction SHA to match exactly.
* Requires exactly one applied branch-hedge well (00e12e8b).
* Requires exactly 4,301 target rows and 14,151 total rows.
* Verifies sample ID order and ensures all predictions are finite `float64` values.
* Automatically aborts if any non-target prediction is altered.

## Acknowledgements
This notebook is a modified fork of the excellent work by Yusuke Togashi (https://www.kaggle.com/yusuketogashi). The underlying hybrid tracking methodologies, contact-gated overrides, and visible-prefix calibration strategies are built upon the robust foundations laid out in their original setup, utilizing the hjyact Version 2 base as the starting point.
"""
...
# ------------------------------------------------------------------
# Explicit external artifact paths used by the CV-backed private hedge
# ------------------------------------------------------------------
V10_FRESH_ARTIFACT_ROOT = '/kaggle/input/datasets/thbdh5765/rogii-v10-fresh-artifacts'
TABICL_MIRROR_ROOT = '/kaggle/input/datasets/needless090/rogii-tabicl-mirror'
OLD_SUBMISSION_ROOT = '/kaggle/input/datasets/nina2025/rogii-03'
MODEL_PACKAGE_ROOT = '/kaggle/input/datasets/pilkwang/rogii-model-package'
REFERENCE_LB_SUBMISSION = '/kaggle/input/notebooks/evgendvorkin/rogii-physics-lb-7-872-v48/submission.csv'

# The notebook first preserves its normal final result, then creates the
# CV-backed private-LB hedge. Set False to keep submission.csv unchanged
# while still writing submission_private_lb_hedge.csv.
RUN_PRIVATE_LB_HEDGE = True
PRIVATE_HEDGE_WRITE_SUBMISSION = True
PRIVATE_HEDGE_PACKAGE_WEIGHT = 0.44

# Cached 773-well CV: no base-model retraining. It reconstructs the
# old Ridge stack, model-package blend and MD-decayed PF correction.
RUN_CACHED_HEDGE_CV = True
CACHED_HEDGE_CV_OUTPUT_DIR = '/kaggle/working/private_hedge_cv'

# This separate full-stack ablation replays expensive physics logic on a
# subset/all train wells. Keep disabled unless the extra runtime is wanted.
# RUN_FULL_STACK_CV_ABLATION remains controlled above.
...
# %% cell 45
# ============================================================
# CV-BACKED PRIVATE-LB HEDGE
#
# Runs after all standard final correction layers.
# Preserves the normal notebook output, then combines:
#   56% current final trajectory
#   44% model-package trajectory
# followed by a distance-decayed PF correction.
#
# This cell does not train a model.
# ============================================================

if bool(globals().get('RUN_PRIVATE_LB_HEDGE', True)):
    from pathlib import Path as _HedgePath
    import json as _hedge_json
    import numpy as _hedge_np
    import pandas as _hedge_pd

    _HEDGE_WORK = _HedgePath('/kaggle/working') if _HedgePath('/kaggle/working').exists() else _HedgePath('.')
    _HEDGE_CURRENT = _HEDGE_WORK / 'submission.csv'
    _HEDGE_PACKAGE = _HEDGE_WORK / 'submission_model_package_only.csv'
    _HEDGE_SAMPLE = _HedgePath(COMPETITION_DATA_ROOT) / 'sample_submission.csv'
    _HEDGE_W = float(globals().get('PRIVATE_HEDGE_PACKAGE_WEIGHT', 0.44))

    if not (0.0 <= _HEDGE_W <= 1.0):
        raise ValueError(f'PRIVATE_HEDGE_PACKAGE_WEIGHT must be in [0,1], got {_HEDGE_W}')
    if not _HEDGE_CURRENT.exists():
        raise RuntimeError(f'Current final submission is missing: {_HEDGE_CURRENT}')
    if not _HEDGE_PACKAGE.exists():
        raise RuntimeError(
            'submission_model_package_only.csv is missing. '
            'RUN_MODEL_PACKAGE_CORRECTION must be enabled and MODEL_PACKAGE_ROOTS must be valid.'
        )

    def _hedge_read_submission(path, label):
        frame = _hedge_pd.read_csv(path)
        if not {'id', 'tvt'}.issubset(frame.columns):
            raise RuntimeError(f'{label}: expected id,tvt columns; got {list(frame.columns)}')
        frame = frame[['id', 'tvt']].copy()
        frame['id'] = frame['id'].astype(str)
        frame['tvt'] = _hedge_pd.to_numeric(frame['tvt'], errors='coerce')
        if frame['id'].duplicated().any():
            raise RuntimeError(f'{label}: duplicate ids')
        if frame['tvt'].isna().any() or not _hedge_np.isfinite(frame['tvt'].to_numpy(float)).all():
            raise RuntimeError(f'{label}: non-finite tvt values')
        return frame

    _hedge_sample = _hedge_pd.read_csv(_HEDGE_SAMPLE)[['id']].copy()
    _hedge_sample['id'] = _hedge_sample['id'].astype(str)
    _hedge_current = _hedge_read_submission(_HEDGE_CURRENT, 'current final')
    _hedge_package = _hedge_read_submission(_HEDGE_PACKAGE, 'model package')

    _hedge_current = _hedge_sample.merge(
        _hedge_current.rename(columns={'tvt': 'tvt_current'}), on='id', how='left'
    )
    _hedge_package = _hedge_sample.merge(
        _hedge_package.rename(columns={'tvt': 'tvt_package'}), on='id', how='left'
    )
    if _hedge_current['tvt_current'].isna().any() or _hedge_package['tvt_package'].isna().any():
        raise RuntimeError('Private hedge input submissions do not cover all sample ids.')

    # test_df is built by the cached Ridge/physics feature section and contains
    # the absolute PF anchor plus distance from the last known survey point.
    _required_pf_cols = {'id', 'pf_ancc', 'last_known_tvt', 'md_since'}
    if 'test_df' not in globals() or not _required_pf_cols.issubset(set(test_df.columns)):
        raise RuntimeError(
            f'test_df with {sorted(_required_pf_cols)} is required for the PF hedge. '
            f'Available={sorted(set(test_df.columns)) if "test_df" in globals() else "test_df missing"}'
        )

    _hedge_pf = test_df[list(_required_pf_cols)].copy()
    _hedge_pf['id'] = _hedge_pf['id'].astype(str)
    if _hedge_pf['id'].duplicated().any():
        raise RuntimeError('test_df contains duplicate ids for the private hedge.')

    _hedge = (
        _hedge_sample
        .merge(_hedge_current, on='id', how='left')
        .merge(_hedge_package, on='id', how='left')
        .merge(_hedge_pf, on='id', how='left')
    )
    for _col in ['tvt_current', 'tvt_package', 'pf_ancc', 'last_known_tvt', 'md_since']:
        _hedge[_col] = _hedge_pd.to_numeric(_hedge[_col], errors='coerce')
    if _hedge[['tvt_current', 'tvt_package', 'pf_ancc', 'last_known_tvt', 'md_since']].isna().any().any():
        _bad = _hedge.loc[
            _hedge[['tvt_current', 'tvt_package', 'pf_ancc', 'last_known_tvt', 'md_since']].isna().any(axis=1),
            'id'
        ].head(10).tolist()
        raise RuntimeError(f'Private hedge has missing aligned values; examples={_bad}')

    _current_v = _hedge['tvt_current'].to_numpy(float)
    _package_v = _hedge['tvt_package'].to_numpy(float)
    _pf_abs = _hedge['pf_ancc'].to_numpy(float)
    _md = _hedge['md_since'].to_numpy(float)

    # Absolute-TVT blending is equivalent to delta blending because all
    # components share the same last-known-TVT anchor.
    _base_v = (1.0 - _HEDGE_W) * _current_v + _HEDGE_W * _package_v

    _pf_w = _hedge_np.zeros(len(_hedge), dtype=float)
    _pf_w[_md <= 10.0] = 1.00
    _pf_w[(_md > 10.0) & (_md <= 25.0)] = 0.95
    _pf_w[(_md > 25.0) & (_md <= 50.0)] = 0.875
    _pf_w[(_md > 50.0) & (_md <= 100.0)] = 0.68
    _pf_w[(_md > 100.0) & (_md <= 200.0)] = 0.46
    _pf_w[(_md > 200.0) & (_md <= 400.0)] = 0.29
    _pf_w[(_md > 400.0) & (_md <= 800.0)] = 0.19
    _pf_w[_md > 800.0] = 0.09

    _pf_valid = _hedge_np.isfinite(_pf_abs) & _hedge_np.isfinite(_md)
    _final_v = _base_v.copy()
    _final_v[_pf_valid] = (
        (1.0 - _pf_w[_pf_valid]) * _base_v[_pf_valid]
        + _pf_w[_pf_valid] * _pf_abs[_pf_valid]
    )
    if not _hedge_np.isfinite(_final_v).all():
        raise RuntimeError('Private hedge produced non-finite predictions.')

    # Preserve all three stages for final-submission selection.
    _normal = _hedge[['id']].copy()
    _normal['tvt'] = _current_v
    _normal.to_csv(_HEDGE_WORK / 'submission_public_lb_safe_6p356.csv', index=False)

    _base = _hedge[['id']].copy()
    _base['tvt'] = _base_v
    _base.to_csv(_HEDGE_WORK / 'submission_private_hedge_before_pf.csv', index=False)

    _out = _hedge[['id']].copy()
    _out['tvt'] = _final_v
    _out.to_csv(_HEDGE_WORK / 'submission_private_lb_hedge.csv', index=False)

    _move_pkg = _base_v - _current_v
    _move_pf = _final_v - _base_v
    _move_total = _final_v - _current_v
    _report = {
        'rows': int(len(_out)),
        'package_weight': float(_HEDGE_W),
        'current_weight': float(1.0 - _HEDGE_W),
        'pf_weight_schedule': {
            '0_10': 1.0, '10_25': 0.95, '25_50': 0.875, '50_100': 0.68,
            '100_200': 0.46, '200_400': 0.29, '400_800': 0.19, '800_inf': 0.09,
        },
        'package_move_rmse': float(_hedge_np.sqrt(_hedge_np.mean(_move_pkg ** 2))),
        'pf_move_rmse': float(_hedge_np.sqrt(_hedge_np.mean(_move_pf ** 2))),
        'total_move_rmse_vs_public_safe': float(_hedge_np.sqrt(_hedge_np.mean(_move_total ** 2))),
        'total_move_p95_abs': float(_hedge_np.quantile(_hedge_np.abs(_move_total), 0.95)),
        'total_move_max_abs': float(_hedge_np.max(_hedge_np.abs(_move_total))),
        'write_submission_csv': bool(globals().get('PRIVATE_HEDGE_WRITE_SUBMISSION', True)),
    }
    with (_HEDGE_WORK / 'private_lb_hedge_report.json').open('w', encoding='utf-8') as _f:
        _hedge_json.dump(_report, _f, indent=2, sort_keys=True)

    if bool(globals().get('PRIVATE_HEDGE_WRITE_SUBMISSION', True)):
        _out.to_csv(_HEDGE_CURRENT, index=False)
        globals()['FINAL_BASE_SOURCE_LABEL'] = 'cv_backed_private_lb_hedge'
        globals()['FINAL_SELECTED_BASE_SOURCE'] = _HEDGE_CURRENT
        print('submission.csv replaced by CV-backed private-LB hedge.', flush=True)
    else:
        print('Private hedge saved separately; submission.csv kept unchanged.', flush=True)

    print('Private-LB hedge report:', _report, flush=True)
else:
    print('Private-LB hedge disabled.')

# %% cell 46
# ============================================================
# 773-WELL CACHED CV FOR THE PRIVATE-LB HEDGE
#
# No base model is retrained. This reconstructs:
#   old 5-model fold-safe Ridge -> model-package blend
#   -> MD-decayed PF correction
#
# Expected reference from the prior validated run: ~9.9622 RMSE.
# This is the CV of the reconstructed hedge components, not a claim
# that every later test-only heuristic has an exact OOF analogue.
# ============================================================

if bool(globals().get('RUN_CACHED_HEDGE_CV', True)):
    from pathlib import Path as _CvHPath
    import json as _cvh_json
    import joblib as _cvh_joblib
    import numpy as _cvh_np
    import pandas as _cvh_pd
    from sklearn.linear_model import Ridge as _CvHRidge
    from sklearn.model_selection import GroupKFold as _CvHGroupKFold

    _CVH_V10 = _CvHPath(V10_FRESH_ARTIFACT_ROOT)
    _CVH_OLD = _CvHPath(RIDGE_ARTIFACT_ROOT)
    _CVH_PKG = _CvHPath(MODEL_PACKAGE_ROOT)
    _CVH_OUT = _CvHPath(CACHED_HEDGE_CV_OUTPUT_DIR)
    _CVH_OUT.mkdir(parents=True, exist_ok=True)

    def _cvh_rmse(y_true, y_pred):
        y_true = _cvh_np.asarray(y_true, dtype=float)
        y_pred = _cvh_np.asarray(y_pred, dtype=float)
        mask = _cvh_np.isfinite(y_true) & _cvh_np.isfinite(y_pred)
        return float(_cvh_np.sqrt(_cvh_np.mean((y_true[mask] - y_pred[mask]) ** 2)))

    # Diagnostics are fold-concatenated. val_indices restores the original
    # compact train-row ordering used by the saved Trainer OOF arrays.
    with _cvh_np.load(_CVH_V10 / 'diagnostics/oof_val_predictions.npz', allow_pickle=False) as _npz:
        _cvh_target_diag = _npz['target'].astype(float)
        _cvh_val_indices = _npz['val_indices'].astype(_cvh_np.int64)

    _cvh_meta_diag = _cvh_pd.read_csv(
        _CVH_V10 / 'diagnostics/oof_val_meta.csv',
        usecols=['well', 'id', 'target', 'last_known_tvt', 'md_since', 'pf_ancc'],
    )
    _cvh_order = _cvh_np.argsort(_cvh_val_indices, kind='stable')
    _cvh_y = _cvh_target_diag[_cvh_order]
    _cvh_meta = _cvh_meta_diag.iloc[_cvh_order].reset_index(drop=True)
    if not _cvh_np.allclose(_cvh_meta['target'].to_numpy(float), _cvh_y, atol=1e-6):
        raise RuntimeError('Cached CV metadata/target alignment failed.')

    _cvh_trainer_files = {
        'catboost_1': _CVH_OLD / 'models/catboost-1/catboostregressor_trainer_20260526193740.pkl',
        'catboost_2': _CVH_OLD / 'models/catboost-2/catboostregressor_trainer_20260526194838.pkl',
        'lightgbm_1': _CVH_OLD / 'models/lightgbm-1/lgbmregressor_trainer_20260526182612.pkl',
        'lightgbm_2': _CVH_OLD / 'models/lightgbm-2/lgbmregressor_trainer_20260526190415.pkl',
        'lightgbm_3': _CVH_OLD / 'models/lightgbm-3/lgbmregressor_trainer_20260526192806.pkl',
    }
    _cvh_trainers, _cvh_oof = {}, {}
    _cvh_verify_rows = []
    for _name, _path in _cvh_trainer_files.items():
        if not _path.exists():
            raise FileNotFoundError(f'Cached trainer missing: {_path}')
        _trainer = _cvh_joblib.load(_path)
        _pred = _cvh_np.asarray(_trainer.oof_preds, dtype=float)
        _score = _cvh_rmse(_cvh_y, _pred)
        _saved = float(_trainer.overall_score)
        if abs(_score - _saved) > 1e-5:
            raise RuntimeError(f'{_name} OOF alignment failed: calculated={_score}, saved={_saved}')
        _cvh_trainers[_name] = _trainer
        _cvh_oof[_name] = _pred
        _cvh_verify_rows.append({'model': _name, 'saved_rmse': _saved, 'recalculated_rmse': _score})

    _cvh_groups = _cvh_meta['well'].astype(str).to_numpy()
    _cvh_fold = _cvh_np.full(len(_cvh_y), -1, dtype=_cvh_np.int8)
    _cvh_dummy = _cvh_np.zeros((len(_cvh_y), 1), dtype=_cvh_np.uint8)
    for _f, (_, _va) in enumerate(_CvHGroupKFold(n_splits=5).split(_cvh_dummy, _cvh_y, groups=_cvh_groups)):
        _cvh_fold[_va] = _f
    if (_cvh_fold < 0).any():
        raise RuntimeError('Cached CV fold reconstruction failed.')

    _cvh_names = list(_cvh_oof)
    _cvh_X = _cvh_np.column_stack([_cvh_oof[n] for n in _cvh_names])
    _cvh_old_ridge = _cvh_np.zeros(len(_cvh_y), dtype=float)
    _cvh_ridge_rows = []
    for _f in range(5):
        _tr = _cvh_fold != _f
        _va = _cvh_fold == _f
        _ridge = _CvHRidge(alpha=1.0, fit_intercept=True)
        _ridge.fit(_cvh_X[_tr], _cvh_y[_tr])
        _cvh_old_ridge[_va] = _ridge.predict(_cvh_X[_va])
        for _i, _name in enumerate(_cvh_names):
            _cvh_ridge_rows.append({
                'fold': _f, 'model': _name, 'weight': float(_ridge.coef_[_i]),
                'intercept': float(_ridge.intercept_),
            })

    _cvh_pkg_gt = _cvh_pd.read_parquet(
        _CVH_PKG / 'oof/train_gt.parquet',
        columns=['id', 'target_delta_from_last_known'],
    )
    if not _cvh_np.array_equal(
        _cvh_pkg_gt['id'].astype(str).to_numpy(),
        _cvh_meta['id'].astype(str).to_numpy(),
    ):
        raise RuntimeError('Model-package OOF ids do not match restored OOF order.')
    _cvh_pkg = _cvh_np.load(_CVH_PKG / 'oof/blend_oof_postprocessed.npy').astype(float)
    if len(_cvh_pkg) != len(_cvh_y):
        raise RuntimeError('Model-package OOF length mismatch.')

    # Fold-safe package weight selection. The prior experiment selected
    # approximately 0.40-0.49 across folds and averages close to 0.44.
    _cvh_alpha_grid = _cvh_np.round(_cvh_np.linspace(0.0, 0.50, 51), 4)
    _cvh_old_pkg = _cvh_np.zeros(len(_cvh_y), dtype=float)
    _cvh_pkg_rows = []
    for _f in range(5):
        _tr = _cvh_fold != _f
        _va = _cvh_fold == _f
        _best_a, _best_s = 0.0, float('inf')
        for _a in _cvh_alpha_grid:
            _candidate = (1.0 - _a) * _cvh_old_ridge[_tr] + _a * _cvh_pkg[_tr]
            _score = _cvh_rmse(_cvh_y[_tr], _candidate)
            if _score < _best_s:
                _best_a, _best_s = float(_a), _score
        _cvh_old_pkg[_va] = (1.0 - _best_a) * _cvh_old_ridge[_va] + _best_a * _cvh_pkg[_va]
        _cvh_pkg_rows.append({
            'fold': _f, 'package_weight': _best_a, 'training_rmse': _best_s,
            'validation_rmse': _cvh_rmse(_cvh_y[_va], _cvh_old_pkg[_va]),
        })

    _cvh_pf_delta = (
        _cvh_meta['pf_ancc'].to_numpy(float)
        - _cvh_meta['last_known_tvt'].to_numpy(float)
    )
    _cvh_md = _cvh_meta['md_since'].to_numpy(float)
    _cvh_pf_valid = _cvh_np.isfinite(_cvh_pf_delta) & _cvh_np.isfinite(_cvh_md)
    _cvh_bins = _cvh_np.array([-_cvh_np.inf, 10, 25, 50, 100, 200, 400, 800, _cvh_np.inf])
    _cvh_bin_id = _cvh_np.digitize(_cvh_md, _cvh_bins[1:-1], right=False)
    _cvh_pf_grid = _cvh_np.round(_cvh_np.linspace(0.0, 1.0, 41), 4)
    _cvh_final = _cvh_np.zeros(len(_cvh_y), dtype=float)
    _cvh_pf_rows = []

    for _f in range(5):
        _tr_fold = _cvh_fold != _f
        _va_fold = _cvh_fold == _f
        _va_positions = _cvh_np.flatnonzero(_va_fold)
        _fold_pred = _cvh_old_pkg[_va_fold].copy()

        for _b in range(len(_cvh_bins) - 1):
            _tr = _tr_fold & _cvh_pf_valid & (_cvh_bin_id == _b)
            _va = _va_fold & _cvh_pf_valid & (_cvh_bin_id == _b)
            _allowed = (
                _cvh_np.array([0.0, 0.025, 0.05, 0.075, 0.10])
                if _cvh_bins[_b] >= 800 else _cvh_pf_grid
            )
            _best_a, _best_s = 0.0, float('nan')
            if int(_tr.sum()) >= 5000:
                _best_s = float('inf')
                for _a in _allowed:
                    _candidate = (1.0 - _a) * _cvh_old_pkg[_tr] + _a * _cvh_pf_delta[_tr]
                    _score = _cvh_rmse(_cvh_y[_tr], _candidate)
                    if _score < _best_s:
                        _best_a, _best_s = float(_a), _score
            _local = _cvh_np.flatnonzero(_va[_va_fold])
            if len(_local):
                _global = _va_positions[_local]
                _fold_pred[_local] = (
                    (1.0 - _best_a) * _cvh_old_pkg[_global]
                    + _best_a * _cvh_pf_delta[_global]
                )
            _cvh_pf_rows.append({
                'fold': _f, 'md_low': float(_cvh_bins[_b]), 'md_high': float(_cvh_bins[_b + 1]),
                'pf_weight': _best_a, 'training_rmse': _best_s,
                'train_rows': int(_tr.sum()), 'valid_rows': int(_va.sum()),
            })
        _cvh_final[_va_fold] = _fold_pred

    _cvh_predictions = {
        'old_ridge_crossfit': _cvh_old_ridge,
        'model_package_post': _cvh_pkg,
        'old_plus_package_crossfit': _cvh_old_pkg,
        'private_hedge_pf_crossfit': _cvh_final,
    }
    _cvh_score_rows = []
    _cvh_fold_rows = []
    for _name, _pred in _cvh_predictions.items():
        _cvh_score_rows.append({
            'model': _name,
            'rmse': _cvh_rmse(_cvh_y, _pred),
            'mae': float(_cvh_np.mean(_cvh_np.abs(_pred - _cvh_y))),
            'bias': float(_cvh_np.mean(_pred - _cvh_y)),
        })
        for _f in range(5):
            _m = _cvh_fold == _f
            _cvh_fold_rows.append({
                'model': _name, 'fold': _f, 'rows': int(_m.sum()),
                'wells': int(_cvh_meta.loc[_m, 'well'].nunique()),
                'rmse': _cvh_rmse(_cvh_y[_m], _pred[_m]),
            })

    _cvh_scores = _cvh_pd.DataFrame(_cvh_score_rows).sort_values('rmse').reset_index(drop=True)
    _cvh_folds = _cvh_pd.DataFrame(_cvh_fold_rows)
    _cvh_verify = _cvh_pd.DataFrame(_cvh_verify_rows)
    _cvh_pkg_report = _cvh_pd.DataFrame(_cvh_pkg_rows)
    _cvh_pf_report = _cvh_pd.DataFrame(_cvh_pf_rows)
    _cvh_ridge_report = _cvh_pd.DataFrame(_cvh_ridge_rows)

    _cvh_scores.to_csv(_CVH_OUT / 'overall_scores.csv', index=False)
    _cvh_folds.to_csv(_CVH_OUT / 'fold_scores.csv', index=False)
    _cvh_verify.to_csv(_CVH_OUT / 'alignment_verification.csv', index=False)
    _cvh_pkg_report.to_csv(_CVH_OUT / 'package_weight_by_fold.csv', index=False)
    _cvh_pf_report.to_csv(_CVH_OUT / 'pf_weight_by_fold_and_md.csv', index=False)
    _cvh_ridge_report.to_csv(_CVH_OUT / 'old_ridge_weights.csv', index=False)

    _cvh_row = _cvh_meta[['well', 'id', 'target', 'last_known_tvt', 'md_since', 'pf_ancc']].copy()
    _cvh_row['fold'] = _cvh_fold
    _cvh_row['pred_old_ridge'] = _cvh_old_ridge.astype(_cvh_np.float32)
    _cvh_row['pred_model_package'] = _cvh_pkg.astype(_cvh_np.float32)
    _cvh_row['pred_old_plus_package'] = _cvh_old_pkg.astype(_cvh_np.float32)
    _cvh_row['pred_private_hedge'] = _cvh_final.astype(_cvh_np.float32)
    _cvh_row['residual_private_hedge'] = (_cvh_final - _cvh_y).astype(_cvh_np.float32)
    _cvh_row.to_parquet(_CVH_OUT / 'oof_private_hedge.parquet', index=False)

    _cvh_per_well = (
        _cvh_row.assign(sq_error=_cvh_row['residual_private_hedge'].astype(float) ** 2)
        .groupby('well', sort=False)
        .agg(
            rows=('id', 'size'),
            fold=('fold', 'first'),
            rmse=('sq_error', lambda x: float(_cvh_np.sqrt(x.mean()))),
            bias=('residual_private_hedge', 'mean'),
        )
        .reset_index()
        .sort_values('rmse', ascending=False)
    )
    _cvh_per_well.to_csv(_CVH_OUT / 'per_well_scores.csv', index=False)

    _cvh_best = _cvh_scores.iloc[0]
    _cvh_summary = {
        'rows': int(len(_cvh_y)),
        'wells': int(_cvh_meta['well'].nunique()),
        'folds': 5,
        'best_model': str(_cvh_best['model']),
        'best_rmse': float(_cvh_best['rmse']),
        'old_ridge_rmse': float(_cvh_rmse(_cvh_y, _cvh_old_ridge)),
        'old_plus_package_rmse': float(_cvh_rmse(_cvh_y, _cvh_old_pkg)),
        'private_hedge_pf_rmse': float(_cvh_rmse(_cvh_y, _cvh_final)),
        'test_time_fixed_package_weight': float(PRIVATE_HEDGE_PACKAGE_WEIGHT),
        'cv_selected_package_weights': [float(x) for x in _cvh_pkg_report['package_weight']],
        'note': (
            'Cached CV covers old Ridge + model-package + PF hedge. '
            'Test-only prefix/overlap/branch heuristics are not all replayed here.'
        ),
    }
    with (_CVH_OUT / 'cv_summary.json').open('w', encoding='utf-8') as _f:
        _cvh_json.dump(_cvh_summary, _f, indent=2, sort_keys=True)

    print('\nCached private-hedge CV:')
    print(_cvh_scores.to_string(index=False))
    print('\nFold scores for private hedge:')
    print(_cvh_folds[_cvh_folds['model'] == 'private_hedge_pf_crossfit'].to_string(index=False))
    print('\nCV outputs:', _CVH_OUT)
else:
    print('Cached private-hedge CV disabled.')

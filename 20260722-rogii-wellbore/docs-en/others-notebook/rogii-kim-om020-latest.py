...
SUBMISSION_PROFILE = 'vp_balanced_modelpkg_010'
...
_profile = {**_profile, 'model_package_gated_max_weight': 0.02, 'run_model_package_correction': True}
...
# --- cell 7 ---
# [fork fix] robust koolbox wheel install
import glob,subprocess,sys
for w in glob.glob('/kaggle/input/**/koolbox*.whl',recursive=True):
    subprocess.run([sys.executable,'-m','pip','install','--no-deps','--force-reinstall',w],check=False)
try:
    import koolbox; print('koolbox OK',koolbox.__file__)
except Exception as e: print('koolbox miss',e)
...
# --- cell 52 ---
# COMPAT SHIM (fork): kernel-env torch build lacks kernels for the allocated GPU
# (cudaErrorNoKernelImageForDevice in the original commit run). Force the saved-model
# TCN correction onto CPU; model weights already load with map_location='cpu'.
import torch as _compat_torch
_compat_torch.cuda.is_available = lambda: False
...
# --- cell 57 ---
# ===== APPENDED CELL: spatial per-well DATUM-OFFSET kriging (CV 9.316->9.209, decorrelated, bounded) =====
# The per-well datum offset b_w (mean residual of train preds) is spatially structured (variogram: ~55% of
# variance within ~1km). Krige it from 773 train wells onto each (hidden) test well via anisotropic gaussian
# weights, add WEIGHT*offset to that well's TVT. Reverts to ~no-op when a well has no near train neighbor.
import os, glob as _g, numpy as _np, pandas as _pd
try:
    _SUB = '/kaggle/working/submission.csv'
    _sub = _pd.read_csv(_SUB)
    # locate lookup (mounted dataset)
    _lk=None
    for _p in _g.glob('/kaggle/input/**/offset_krige_lookup.csv', recursive=True):
        _lk=_pd.read_csv(_p); break
    if _lk is None: raise FileNotFoundError('offset_krige_lookup.csv not mounted')
    _CX=_lk['cx'].to_numpy(float); _CY=_lk['cy'].to_numpy(float); _MR=_lk['b_w'].to_numpy(float); _WL=_lk['well'].astype(str).to_numpy()
    TH=2.2166; ANISO=2.0; RNG=500.0; WEIGHT=0.30; K=25            # locked config (CV-validated)
    ux,uy=_np.cos(TH),_np.sin(TH); vx,vy=-uy,ux
    _px=_CX*ux+_CY*uy; _py=(_CX*vx+_CY*vy)/ANISO                   # anisotropic train coords
    # candidate roots for mounted well CSVs
    _roots=set()
    for _c in _g.glob('/kaggle/input/**/*__horizontal_well.csv', recursive=True):
        _roots.add(os.path.dirname(_c))
    def _find_hw(_w):
        for _r in _roots:
            _f=os.path.join(_r, _w+'__horizontal_well.csv')
            if os.path.exists(_f): return _f
        return None
    _sub['__w']=_sub['id'].astype(str).str[:8]
    _applied=0; _skip=0; _shift={}
    for _w in _sub['__w'].unique():
        _f=_find_hw(_w)
        if _f is None: _skip+=1; continue
        try:
            _hw=_pd.read_csv(_f, usecols=['X','Y','TVT_input'])
        except Exception:
            _hw=_pd.read_csv(_f)
        _tin=_pd.to_numeric(_hw['TVT_input'],errors='coerce').to_numpy(float)
        _ev=~_np.isfinite(_tin)
        _qx=_np.nanmean(_hw['X'].to_numpy(float)[_ev]) if _ev.any() else _np.nanmean(_hw['X'].to_numpy(float))
        _qy=_np.nanmean(_hw['Y'].to_numpy(float)[_ev]) if _ev.any() else _np.nanmean(_hw['Y'].to_numpy(float))
        if not (_np.isfinite(_qx) and _np.isfinite(_qy)): _skip+=1; continue
        _qpx=_qx*ux+_qy*uy; _qpy=(_qx*vx+_qy*vy)/ANISO
        _d2=(_px-_qpx)**2+(_py-_qpy)**2
        _keep=_WL!=_w                                             # exclude exact self-id (defensive)
        _d2k=_d2[_keep]; _MRk=_MR[_keep]
        _order=_np.argsort(_d2k)[:K]; _dk=_d2k[_order]; _mk=_MRk[_order]
        _wt=_np.exp(-_dk/(2*RNG**2)); _sw=_wt.sum()
        if _sw<1e-9: _skip+=1; continue
        _off=float(_np.sum(_wt*_mk)/_sw)
        _shift[_w]=WEIGHT*_off; _applied+=1
    if _shift:
        _sub['tvt']=_sub['tvt']+_sub['__w'].map(_shift).fillna(0.0).to_numpy(float)
    _sub=_sub.drop(columns='__w')
    _sub.to_csv(_SUB, index=False)
    _sh=_np.array(list(_shift.values())) if _shift else _np.array([0.0])
    print('[offset-krige] applied %d wells, skipped %d; shift mean %.3f std %.3f max|%.3f|'%(
        _applied,_skip,_sh.mean(),_sh.std(),_np.abs(_sh).max()))
except Exception as _e:
    import traceback as _tb; print('[offset-krige] SKIPPED (safe):', _e); _tb.print_exc()
# =========================================================================================================
